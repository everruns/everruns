...
/// Where this editor sits in the navigation. The endpoint itself is the same
/// row whether it is reached through the App page or through an agent's
/// Integrations tab (EVE-1009), so only the surrounding links differ — the
/// editor is shared rather than duplicated.
export interface ChannelEditorNav {
  breadcrumbs: BreadcrumbItem[];
  /// Where Discard, Save and Delete return to, and what Back links to.
  returnHref: string;
  returnLabel: string;
}
...
export function ChannelEditor({
  ...
  }, [app, canManage, nav.returnHref, policiesLoading, router]);
  ...
  if (!app || !channel) {
    return (
      <ResourceNotFound
        ...
        backHref={nav.returnHref}
        backLabel={`Back to ${nav.returnLabel}`}
  ...
  return (
    <PageContainer>
      <PageBreadcrumb items={[...nav.breadcrumbs, { label: channelTitle(formState) }]} />
      ...
      <PageMasthead
        ...
        actions={
          <>
            ...
            <Button type="button" variant="outline" onClick={() => router.push(nav.returnHref)}>
              Discard
            </Button>
          </>
        }
      ...
      <PageFooter>
        <BackLink href={nav.returnHref}>Back to {nav.returnLabel}</BackLink>
      </PageFooter>
    </PageContainer>
  );
}
