...
export default function DurableDashboardPage() {
  ...
  const stats = [
    ...
    {
      title: "Workflows",
      value: (health?.pending_workflows || 0) + (health?.running_workflows || 0),
      description: `${health?.pending_workflows || 0} pending, ${health?.running_workflows || 0} running, ${health?.completed_workflows || 0} completed, ${health?.failed_workflows || 0} failed`,
      icon: Workflow,
      color: "text-green-600",
    },
    {
      title: "Tasks",
      value: (health?.pending_tasks || 0) + (health?.claimed_tasks || 0),
      description: `${health?.pending_tasks || 0} pending, ${health?.claimed_tasks || 0} claimed, ${health?.completed_tasks || 0} completed, ${health?.failed_tasks || 0} failed`,
      icon: Clock,
