...
// All routes are org-scoped: /v1/orgs/{org}/agents/{agentId}/sessions/{sessionId}/events
...
export async function listEvents(
  ...
  const response = await api.get<ListResponse<Event>>(
    `/v1/orgs/${org}/agents/${agentId}/sessions/${sessionId}/events`
  );
...
export function getSseUrl(org: string, agentId: string, sessionId: string, sinceId?: string): string {
  const baseUrl = getDirectBackendUrl();
  const params = sinceId ? `?since_id=${sinceId}` : "";
  return `${baseUrl}/v1/orgs/${org}/agents/${agentId}/sessions/${sessionId}/sse${params}`;
}
