...
const MAX_MAX_SPEND_USD: f64 = 10.0;
...
impl Tool for ProbeModelTool {
    ...
    async fn execute_with_context(
        ...
        for task in &tasks {
            // Check the budget before each probe so a zero (or already-exhausted)
            // budget results in zero paid requests rather than one.
...
fn bounded_probe_max_spend_usd(arguments: &Value) -> f64 {
    ...
    if spend.is_finite() {
        spend.clamp(0.0, MAX_MAX_SPEND_USD)
    } else {
