...
mod tests {
    ...
    fn config() -> LlmCallConfig {
        let mut config = LlmCallConfig::new("accounts/fireworks/models/example");
    ...
    fn expected_profile(
        ...
        let mut expected = json!({"name":name,"family":family,"attachment":image,"reasoning":false,"temperature":true,"tool_call":tools,"structured_output":true,"open_weights":true,"modalities":{"input":if image { vec!["text","image"] } else { vec!["text"] },"output":["text"]},"tool_search":false,"supports_phases":false,"mid_conversation_system":false});
