...
pub struct FeatureFlags {
    ...
    /// Authenticated MCP endpoint and remote-control tool surface. Experimental.
    /// Requires deployment enablement and per-org opt-in.
...
pub const API_FEATURE_FLAG_DEFINITIONS: &[FeatureFlagDefinition] = &[
    ...
    FeatureFlagDefinition {
        name: "mcp_endpoint",
        label: "MCP endpoint",
        description: "Allows authenticated MCP clients to connect to Everruns and use the MCP tool surface.",
        experimental: true,
    },
    FeatureFlagDefinition {
        name: "webmcp",
...
mod tests {
    ...
    #[test]
    fn test_mcp_endpoint_is_in_api_feature_flag_catalog() {
        assert!(
            API_FEATURE_FLAG_DEFINITIONS
                .iter()
                .any(|definition| definition.name == "mcp_endpoint")
        );
    }
