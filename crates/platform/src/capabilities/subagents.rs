...
async fn spawn_agent_subagent_impl(
    ...
    if let Some(ref bp_id) = blueprint_param {
        ...
        //
        // THREAT[TM-AUTHZ]: this runs after the capability gate above. The error
        // echoes the blueprint's schema so the model can correct its own call,
        // which is only safe to disclose once the session is known to have the
        // contributing capability enabled.
