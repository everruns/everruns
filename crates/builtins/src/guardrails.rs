...
/// Bytes of stage content sent to a judge (utility LLM or decisions).
...
const DECISIONS_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(10);
...
const MAX_DECISIONS_QUESTIONS: usize = 24;
...
async fn run_decisions_batch(
    service: &Arc<dyn DecisionsService>,
    ...
    let mut request = DecisionRequest::new(serde_json::Value::Object(state))
        .with_metadata("purpose", "guardrails")
        .with_metadata("stage", stage.as_str());

    // Decisions checks: one noul each, thresholded by the check's own config.
    ...
    {
        if request.len() >= MAX_DECISIONS_QUESTIONS {
            break;
        }
    ...
    let judgment = match tokio::time::timeout(DECISIONS_TIMEOUT, service.evaluate(request)).await {
    ...
    for (id, (index, threshold)) in thresholds {
        let Some(DecisionAnswer::Noul { probability }) = judgment.get(&id) else {
...
/// configured no decisions.
async fn stage_decisions(
    context_service: Option<&Arc<dyn DecisionsService>>,
    compiled: &CompiledGuardrails,
    stage: GuardrailStage,
    tool_name: Option<&str>,
    content: &str,
    ...
    match context_service {
        Some(service) => {
            run_decisions_batch(service, compiled, stage, tool_name, content, content_cap).await
        }
        None => {
            if compiled.has_jev_checks_for_stage(stage) {
                tracing::warn!(
                    stage = %stage.as_str(),
                    "guardrails: jev-engine checks skipped — no decisions \
...
/// Timeout for a single moderation decisions call. Fail-open on expiry —
...
/// Bytes of assistant text sent to the moderation decisions.
const MODERATION_CONTENT_CAP: usize = 4_000;

/// System prompt sent to the utility LLM for moderation scoring.
const MODERATION_SYSTEM_PROMPT: &str = r#"You are a content-moderation decisions for an AI agent runtime.
...
/// decisions. Returns `Some(GuardrailAction::Block)` when any requested
...
mod tests {
    ...
    async fn run_moderation_seam_with(
        config: &serde_json::Value,
        service: Option<Arc<dyn UtilityLlmService>>,
        judgment: Option<Arc<dyn DecisionsService>>,
        text: &str,
    ) -> GuardrailDecision {
    ...
    async fn judge_pre_tool_hook_skipped_when_service_not_configured() {
        ...
        // (e.g. DisabledUtilityLlmService). Decisions checks must be silently skipped.
    ...
    /// Stub decisions: answers every noul with `noul`, every score with
    ...
    struct StubJudgment {
        ...
        fail: bool,
        requests: std::sync::Mutex<Vec<DecisionRequest>>,
    }
    ...
    impl StubJudgment {
        ...
        fn recorded(&self) -> Vec<DecisionRequest> {
            self.requests.lock().expect("stub lock").clone()
        }
    }

    #[async_trait]
    impl DecisionsService for StubJudgment {
        fn is_configured(&self) -> bool {
            true
        }
        ...
        ) -> crate::Result<everruns_core::DecisionOutcome> {
            self.requests
                .lock()
                .expect("stub lock")
                .push(request.clone());
    ...
    fn decisions_ctx(service: Arc<StubJudgment>) -> ToolContext {
        ToolContext::new(SessionId::new()).with_decisions(service)
    }
    ...
    async fn jev_engine_allows_below_the_threshold() {
        ...
        let decision = hooks[0]
            .before_exec(
                ...
                &tool_def(),
                &decisions_ctx(StubJudgment::noul(0.49)),
            )
            .await;
        assert!(matches!(decision, PreToolUseDecision::Continue(_)));
    }
    ...
    async fn jev_engine_honors_a_custom_threshold() {
        ...
        let allowed = hooks[0]
            .before_exec(
                ...
                &tool_def(),
                &decisions_ctx(StubJudgment::noul(0.8)),
            )
            .await;
        assert!(
        ...
        let blocked = hooks[0]
            .before_exec(
                tool_call("delete_record", json!({})),
                &tool_def(),
                &decisions_ctx(StubJudgment::noul(0.95)),
            )
            .await;
    ...
    async fn the_request_carries_stage_tool_and_content_as_data() {
        ...
        assert!(
            ...
            "a guardrail never names a model; the deployment's decisions picks it"
        );
        let (_, question) = &request.questions[0];
        let DecisionQuestion::Noul { instructions, .. } = question else {
    ...
    async fn jev_and_utility_engines_coexist_on_one_stage() {
        ...
        let ctx = ToolContext::new(SessionId::new())
            .with_decisions(StubJudgment::noul(0.0))
    ...
    async fn jev_checks_fail_open_without_a_service() {
        ...
        assert!(
            matches!(decision, PreToolUseDecision::Continue(_)),
            "a missing decisions must never wedge a turn"
        );
    }

    #[tokio::test]
    async fn jev_checks_fail_open_when_the_deployment_key_is_unset() {
        // `UTILITY_TYPESAFE_API_KEY` unset resolves to DisabledDecisionsService,
        // which is wired in like any other: present, but not configured.
        let hooks = GuardrailsCapability.pre_tool_use_hooks_with_config(&judge_config(json!({})));
        let ctx = ToolContext::new(SessionId::new())
            .with_decisions(Arc::new(everruns_core::DisabledDecisionsService));
        let decision = hooks[0]
            .before_exec(tool_call("delete_record", json!({})), &tool_def(), &ctx)
            .await;
        assert!(
            matches!(decision, PreToolUseDecision::Continue(_)),
            "an unconfigured decisions must never wedge a turn"
        );
    }
    ...
    async fn jev_checks_fail_open_when_the_service_errors() {
        ...
        let decision = hooks[0]
            .before_exec(
                tool_call("delete_record", json!({})),
                &tool_def(),
                &decisions_ctx(StubJudgment::failing()),
            )
            .await;
        assert!(matches!(decision, PreToolUseDecision::Continue(_)));
    }
    ...
    async fn an_unanswered_question_fails_open_without_affecting_the_others() {
        ...
        let decision = hooks[0]
            .before_exec(
                tool_call("any_tool", json!({})),
                &tool_def(),
                &decisions_ctx(StubJudgment::silent_on(&["judge_0"])),
            )
            .await;
        assert!(
            matches!(decision, PreToolUseDecision::Block { .. }),
    ...
    async fn jev_engine_guards_the_tool_output_stage() {
        ...
        hooks[0]
            .after_exec(
                ...
                &tool_def(),
                &mut result,
                &decisions_ctx(StubJudgment::noul(0.99)),
            )
            .await;
    ...
    async fn advisory_mode_downgrades_a_jev_block_to_a_log() {
        ...
        let decision = hooks[0]
            .before_exec(
                tool_call("delete_record", json!({})),
                &tool_def(),
                &decisions_ctx(StubJudgment::noul(1.0)),
            )
            .await;
        assert!(matches!(decision, PreToolUseDecision::Continue(_)));
    }
