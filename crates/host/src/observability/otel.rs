...
pub use super::otel_config::TraceConventions;
use super::otel_config::record_content_from_env;
...
impl OtelEventListener {
    ...
    pub fn new() -> Self {
        Self::with_global(record_content_from_env(), TraceConventions::from_env())
    }
    ...
    pub fn with_record_content(record_content: bool) -> Self {
        Self::with_global(record_content, TraceConventions::from_env())
    }

    /// Listener using the application-installed global tracer provider.
    pub fn with_global(record_content: bool, conventions: TraceConventions) -> Self {
        Self::with_tracer(
    ...
    /// Listener backed by an application-owned SDK tracer provider.
    ///
    /// Retaining the provider lets the listener force completed spans through
    /// its processors during framework shutdown.
    pub fn with_tracer_provider(
        tracer_provider: SdkTracerProvider,
        record_content: bool,
        conventions: TraceConventions,
    ) -> Self {
        let tracer = BoxedTracer::new(Box::new(opentelemetry::trace::TracerProvider::tracer(
            &tracer_provider,
            TRACER_NAME,
        )));
        Self {
