...
pub fn credentials_path() -> Result<PathBuf> {
    let config_dir =
        crate::user_dirs::config_dir().ok_or_else(|| anyhow!("Could not determine config directory"))?;
...
mod tests {
    ...
    fn test_credentials_path_macos_uses_application_support() {
        let path = credentials_path().unwrap();
        let expected_prefix =
            crate::user_dirs::config_dir().expect("user_dirs::config_dir() should succeed on macOS");
        assert!(
