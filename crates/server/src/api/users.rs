...
use everruns_core::validate_project_public_id;
...
pub async fn switch_project(
    ...
    // Projects are gated: when the org has not enabled the flag the switcher does
    // not exist, so the endpoint is indistinguishable from an unknown route.
    if !org.feature_flags.projects {
        return Err(StatusCode::NOT_FOUND);
    }
    if !validate_project_public_id(&req.project_id) {
        tracing::warn!(project_id = %req.project_id, "Invalid project ID format");
        return Err(StatusCode::BAD_REQUEST);
    }
    ...
    if project.is_none() {
        tracing::warn!(
            ...
            org = %org.public_id,
            "User attempted to switch to a project not in their org"
        );
        return Err(StatusCode::NOT_FOUND);
    }
