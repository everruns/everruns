...
//
// What these cover: a caller that reads only text still learns what was asked,
// a caller that reads data gets the same question typed and schema-declared, an
// answer submitted as a DataPart resumes the parked turn, and a credential is
// never projected as something a remote agent could fill in.
