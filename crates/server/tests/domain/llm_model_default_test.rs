...
//!
//! Run with: cargo test -p everruns-server --test domain llm_model_default_test::
...
async fn test_create_installed_model() {
    ...
    let m1 = db
        .create_model(TEST_ORG_ID, model_input(pid, "model-a", true))
        .await
        .expect("create model-a");
    assert!(m1.enabled);

    let m2 = db
        .create_model(TEST_ORG_ID, model_input(pid, "model-b", false))
        .await
...
async fn test_multiple_models_can_be_installed() {
    ...
    let m1 = db
        ...
        .expect("create model-a");
    let m2 = db
        .create_model(TEST_ORG_ID, model_input(pid, "model-b", true))
        .await
        .expect("create model-b");
    ...
    let m1_fetched = db
        .get_model(TEST_ORG_ID, m1.id.into())
        .await
        .expect("get model-a")
        .expect("model-a exists");
    let m2_fetched = db
        .get_model(TEST_ORG_ID, m2.id.into())
        .await
        .expect("get model-b")
...
async fn test_update_installed_flag() {
    ...
    let m1 = db
        .create_model(TEST_ORG_ID, model_input(pid, "model-a", false))
        .await
        .expect("create model-a");
    assert!(!m1.enabled);
...
async fn test_seed_upsert_sets_installed() {
    ...
    let result = db
        .create_model_with_id(TEST_ORG_ID, seed_id, model_input(pid, "seed-model", true))
        .await
        .expect("seed model");
...
async fn test_get_default_model_returns_none_without_org_settings() {
    ...
    db.create_model(TEST_ORG_ID, model_input(pid, "model-a", true))
        .await
        .expect("create model-a");
...
async fn test_delete_model_that_is_org_default() {
    ...
    // Delete the model
    let deleted = db
        .delete_model(TEST_ORG_ID, m1.id.into())
        .await
        .expect("delete model");
    assert!(deleted);
