-- Capability Secrets Table
-- Stores encrypted secrets for capability configurations (e.g., API keys for web search providers)
...
CREATE INDEX IF NOT EXISTS idx_capability_secrets_agent_id ON capability_secrets(agent_id);

-- Index for org-level queries (e.g., listing all secrets for an org)
CREATE INDEX IF NOT EXISTS idx_capability_secrets_org_id ON capability_secrets(org_id);

-- Combined index for the common lookup pattern
CREATE INDEX IF NOT EXISTS idx_capability_secrets_lookup
    ON capability_secrets(agent_id, capability_id);
