----------------------------------------------------------------------
-- 011: Agent addressable name
----------------------------------------------------------------------

-- Makes agent `name` the URL/CLI-friendly addressable identifier (slug),
-- mirroring the harness pattern from migration 010.
...
-- Step 2: Convert name to slug format (lowercase, non-alphanumeric-hyphen -> hyphens,
-- collapse consecutive hyphens, trim hyphens).
-- Fall back to 'agent-<short-id>' if slugification yields empty string.
UPDATE agents
SET name = CASE
    WHEN TRIM(BOTH '-' FROM
        regexp_replace(
