...
impl EventService {
    ...
    ///
    /// Idempotency: If an event with the same exec_id and event_type already exists
    /// for this session, returns the existing event instead of creating a duplicate.
    /// This prevents duplicate events when activities are retried.
    ///
    /// Note: The event ID is generated by the database, not by Rust.
    pub async fn emit(&self, request: EventRequest) -> Result<Event> {
        // Idempotency check: if exec_id is present, check for existing event
        if let Some(exec_id) = request.context.exec_id {
            if let Some(existing) = self
                .db
                .find_event_by_exec_id(request.session_id, exec_id, &request.event_type)
                .await?
            {
                tracing::debug!(
                    ...
                    "Skipping duplicate event emission - event already exists for exec_id"
                );
                return Ok(Self::row_to_event(existing));
            }
        }
    ...
    /// Returns the count of successfully stored events (excluding duplicates).
    ///
    /// This method is optimized for bulk event ingestion from workers.
    /// Idempotency checking is applied to each event.
    /// All registered listeners are notified for each newly created event.
    pub async fn emit_batch(&self, requests: Vec<EventRequest>) -> Result<i32> {
        let mut count = 0i32;

        for request in requests {
            // Use emit() which includes idempotency checking
