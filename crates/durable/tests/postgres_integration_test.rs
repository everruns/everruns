...
use everruns_durable::persistence::{
    ...
    TaskFailureOutcome, TaskFilter, TraceContext, WorkerFilter, WorkerInfo, WorkflowEventStore,
    WorkflowStatus,
};
...
// ============================================
// Task Count Verification Tests (Comprehensive Duplicate Prevention)
// ============================================

/// Test that verifies only ONE next activity is scheduled after a race condition
///
/// This is the key test for the duplicate atoms bug. The scenario:
/// 1. Create workflow with "reason" task
/// 2. Worker A claims and executes
/// 3. Task reclaimed (heartbeat timeout)
/// 4. Worker B claims and completes
/// 5. Worker A tries to complete (rejected)
/// 6. Verify: EXACTLY ONE "act" task exists (not zero, not two)
#[tokio::test]
async fn test_exactly_one_next_task_after_race_condition() {
    let store = create_test_store().await;
    let workflow_id = Uuid::now_v7();

    store
        .create_workflow(workflow_id, "counting_test", json!({}), None)
        .await
        .unwrap();

    // Create initial "reason" task
    let task_id = store
        .enqueue_task(TaskDefinition {
            workflow_id,
            activity_id: "reason_1".to_string(),
            activity_type: "reason".to_string(),
            input: json!({"session_id": "test"}),
            options: ActivityOptions::default(),
        })
        .await
        .unwrap();
    ...
    // Worker B schedules next activity (this is what happens in the real workflow)
    store
        .enqueue_task(TaskDefinition {
            workflow_id,
            activity_id: "act_1".to_string(),
            activity_type: "act".to_string(),
            input: json!({"tool_calls": []}),
            options: ActivityOptions::default(),
        })
        .await
        .unwrap();

    // Worker A tries to complete (should fail)
    let result_a = store
        .complete_task(task_id, "worker-A", json!({"has_tool_calls": true}))
        .await;
    assert!(matches!(result_a, Err(StoreError::TaskNotOwned(_))));

    // CRITICAL: Worker A should NOT schedule next activity because complete failed
    // In buggy code, Worker A would also call enqueue_task creating a duplicate

    // Verify: Count tasks for this workflow - should be exactly 1 "act" task
    let all_tasks = store
        .list_tasks(
            TaskFilter {
                workflow_id: Some(workflow_id),
    ...
    let act_tasks: Vec<_> = all_tasks
        .iter()
        .filter(|t| t.activity_type == "act")
        .collect();

    assert_eq!(
        act_tasks.len(),
        1,
        "Expected exactly 1 'act' task, but found {}. This indicates duplicate scheduling!",
        act_tasks.len()
    );
...
/// Test multiple rapid task completions don't cause duplicates
///
/// Scenario: Multiple workers try to complete the same task nearly simultaneously
#[tokio::test]
async fn test_concurrent_completions_no_duplicates() {
    let store = create_test_store().await;
    let workflow_id = Uuid::now_v7();

    store
        .create_workflow(workflow_id, "concurrent_test", json!({}), None)
        .await
        .unwrap();
    ...
    // Simulate: Multiple workers try to complete concurrently
    // (In reality this happens when task is reclaimed and original completes late)
    let store1 = store.clone();
    let store2 = store.clone();
    let store3 = store.clone();

    let (r1, r2, r3) = tokio::join!(
        store1.complete_task(task_id, "worker-1", json!({"result": 1})),
        store2.complete_task(task_id, "worker-2", json!({"result": 2})),
        store3.complete_task(task_id, "worker-3", json!({"result": 3})),
    );

    // Count successful completions - should be exactly 1
    let successes = [&r1, &r2, &r3].iter().filter(|r| r.is_ok()).count();

    assert_eq!(
        successes, 1,
        "Expected exactly 1 successful completion, got {}",
        successes
    );

    // The other two should be TaskNotOwned
    let not_owned_count = [r1, r2, r3]
        .iter()
        .filter(|r| matches!(r, Err(StoreError::TaskNotOwned(_))))
        .count();

    assert_eq!(
        not_owned_count, 2,
        "Expected 2 TaskNotOwned errors, got {}",
        not_owned_count
    );
...
/// Test that simulates the full workflow loop with potential race conditions
///
/// This test simulates: user message → process_input → reason → act → reason
/// with a race condition during the act → reason transition
#[tokio::test]
async fn test_workflow_loop_with_race_condition() {
    let store = create_test_store().await;
    let workflow_id = Uuid::now_v7();
    ...
    // 1. Create process_input task
    let input_task = store
        .enqueue_task(TaskDefinition {
            workflow_id,
    ...
    // Complete process_input normally
    store
        .claim_task("worker-1", &["process_input".to_string()], 1)
        .await
        .unwrap();
    store
        .complete_task(input_task, "worker-1", json!({}))
        .await
        .unwrap();

    // 2. Schedule and complete reason task
    let reason_task = store
        .enqueue_task(TaskDefinition {
            workflow_id,
            activity_id: "reason_1".to_string(),
            activity_type: "reason".to_string(),
    ...
    store
        ...
        .unwrap();
    store
        .complete_task(reason_task, "worker-1", json!({"has_tool_calls": true}))
        .await
        .unwrap();

    // 3. Schedule act task - THIS IS WHERE THE BUG OCCURS
    let act_task = store
        .enqueue_task(TaskDefinition {
            workflow_id,
            activity_id: "act_1".to_string(),
            activity_type: "act".to_string(),
            input: json!({"tool_calls": [{"id": "1", "name": "test"}]}),
            options: ActivityOptions::default(),
        })
        .await
        .unwrap();

    // Worker A claims act task
    store
    ...
    store
        ...
        .unwrap();
    store
        .complete_task(act_task, "worker-B", json!({"success": true}))
        .await
        .unwrap();

    // Worker B schedules next reason task
    store
        .enqueue_task(TaskDefinition {
            workflow_id,
    ...
    // Worker A finishes late and tries to complete (should fail)
    let worker_a_result = store
        .complete_task(act_task, "worker-A", json!({"success": true}))
        .await;
    assert!(
        matches!(worker_a_result, Err(StoreError::TaskNotOwned(_))),
        "Worker A should be rejected"
    );

    // VERIFICATION: Count reason tasks for this workflow
    let all_tasks = store
        .list_tasks(
            TaskFilter {
                workflow_id: Some(workflow_id),
    ...
    let reason_tasks: Vec<_> = all_tasks
        .iter()
        .filter(|t| t.activity_type == "reason")
        .collect();

    // Should have exactly 2 reason tasks: reason_1 (completed) + reason_2 (pending)
    // NOT 3 (which would happen if Worker A also scheduled one)
    assert_eq!(
        reason_tasks.len(),
        2,
        "Expected exactly 2 reason tasks (initial + follow-up), got {}. Activity IDs: {:?}",
        reason_tasks.len(),
        reason_tasks
            .iter()
            .map(|t| &t.activity_id)
...
/// Test edge case: task status is Pending when worker tries to complete
///
/// This can happen if the task was reclaimed but not yet claimed by another worker
#[tokio::test]
async fn test_complete_pending_task_rejected() {
    let store = create_test_store().await;
    let workflow_id = Uuid::now_v7();

    store
        .create_workflow(workflow_id, "pending_test", json!({}), None)
        .await
        .unwrap();
    ...
    // Manually set task back to pending (simulating reclaim without re-claim)
    sqlx::query(
        "UPDATE durable_task_queue SET status = 'pending', claimed_by = NULL WHERE id = $1",
    )
    .bind(task_id)
    .execute(store.pool())
    .await
    ...
    // Original worker tries to complete - should fail
    let result = store.complete_task(task_id, "worker-1", json!({})).await;
    assert!(
        matches!(result, Err(StoreError::TaskNotOwned(_))),
...
/// Test edge case: Task claimed by different worker than who tries to complete
#[tokio::test]
async fn test_complete_with_mismatched_worker_id() {
    let store = create_test_store().await;
    let workflow_id = Uuid::now_v7();

    store
        .create_workflow(workflow_id, "mismatch_test", json!({}), None)
        .await
        .unwrap();
    ...
    // Verify database shows worker-A as owner
    let task_info = store
        .list_tasks(
            TaskFilter {
                workflow_id: Some(workflow_id),
        ...
        .await
        .unwrap();
    assert_eq!(task_info[0].claimed_by.as_deref(), Some("worker-A"));

    // Worker-B tries to complete (impersonation attempt)
    let result = store.complete_task(task_id, "worker-B", json!({})).await;
    assert!(
        matches!(result, Err(StoreError::TaskNotOwned(_))),
        "Worker-B should not be able to complete Worker-A's task: {:?}",
        result
    );

    // Worker-A can still complete
    store
        .complete_task(task_id, "worker-A", json!({}))
        .await
        .unwrap();
