...
//! input, and the model answers. Models with `clear_at` receive a turn-scoped
//! system message; other models receive the existing user-message fallback. On
//! the next request that answer is history, and the facts block before it has
...
pub(super) fn mark_turn_scoped(
    ...
    if supported && source.is_turn_scoped_reminder() {
        target.mark_turn_scoped_system();
    }
}
...
pub(super) fn interleave_facts(
    ...
    turn_scoped_system: bool,
) -> (Vec<RuntimeMessage>, usize) {
    let fact_message = |block| {
        if turn_scoped_system {
            RuntimeMessage::turn_scoped_system(block)
        } else {
            RuntimeMessage::user(block)
        }
    };
    let answered =
    ...
    for (index, message) in messages.into_iter().enumerate() {
        ...
        {
            placed.push(fact_message(block));
        }
    }
...
mod tests {
    ...
    fn a_block_follows_every_answered_input_as_of_that_input() {
        ...
        let (placed, volatile) = interleave_facts(history, render, false);
        assert_eq!(
            texts(&placed),
            [
                "task",
    ...
    fn the_next_request_only_appends() {
        ...
        let (one, _) = interleave_facts(turn_one, render, false);
        let (two, _) = interleave_facts(turn_two, render, false);
    ...
    fn no_dynamic_facts_leaves_the_messages_alone() {
        ...
        let (placed, volatile) = interleave_facts(history, |_| None, false);
        assert_eq!(texts(&placed), ["task", "answer"]);
        assert_eq!(volatile, 0);
    }

    #[test]
    fn clear_at_capable_models_get_turn_scoped_system_facts() {
        use RuntimeMessageRole::*;
        let history = vec![message(User, "task", 0)];
        let (placed, volatile) = interleave_facts(history, render, true);
        assert_eq!(placed[1].role, System);
        assert!(placed[1].is_turn_scoped_reminder());
        assert_eq!(placed[1].text(), Some("<facts>10:00</facts>"));
        assert_eq!(volatile, 0);
    }
}
