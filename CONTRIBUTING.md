...
- If your current git config already points to a real human identity, you can use it as-is.
- If your current git config is missing or agent-like, set `GIT_USER_NAME` and `GIT_USER_EMAIL`, then run `git config user.name "$GIT_USER_NAME"` and `git config user.email "$GIT_USER_EMAIL"` before committing.
...
- The `just pre-push` script rejects agent-like git identities and outgoing commits authored under agent-like identities
...
See [`knowledge/test-cases/`](./knowledge/test-cases/) for manual test cases (format in [`knowledge/test-cases/format.md`](./knowledge/test-cases/format.md)).
