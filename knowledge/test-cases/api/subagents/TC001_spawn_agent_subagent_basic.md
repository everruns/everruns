---
type: Test Case
title: "TC001: Spawn Subagent - Basic"
description: "Verify that an agent with the `subagents` capability can spawn a subagent via the `spawn_agent` tool, creating a child session with `parent_session_id` set and returning `subagent_id`, `name`, and `status` in the tool..."
tags:
  - everruns
  - test-case
  - api
