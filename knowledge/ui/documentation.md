...
1. Every canonical prose docs page must have a Markdown representation at
   `<route>/index.md` (and the home page at `/index.md`). Each file must carry
   the page title, description, canonical `Source:` URL, absolute links, and the
   same rendered MDX cleanup as the whole-corpus output. Generated OpenAPI
   reference pages use `/api/openapi.json` as their machine-readable form
2. `/llms.txt` must be an index, not a pointer to two dumps: the three ways to
...
3. Every documentation set in `astro.config.mjs` must correspond to a sidebar
   topic, so a reader with a narrow question can take one set — the complete
   text is around 250k tokens
4. Every page in every text output must carry a `Source:` line with its
...
5. `llms-small.txt` must be meaningfully smaller than `llms-full.txt` (at most
...
6. The REST API is excluded from the Markdown and corpus text outputs because
   the OpenAPI schema is the better machine-readable form — so the schema must
   actually be served, at `/api/openapi.json`
   (`apps/docs/scripts/copy-openapi.mjs`)
7. `robots.txt` must allow the AI crawlers that respect it
8. `apps/docs/scripts/verify-llms.mjs` enforces all of the above at postbuild,
   including that each prose sitemap page has an `index.md` and each `Source:`
   URL resolves to a built page
9. Requirements 2-5 need behavior the upstream plugin does not have; the delta
   lives in `apps/docs/patches/starlight-llms-txt@0.11.0.patch` and must be
   re-cut when the plugin is upgraded. The per-page route lives in
   `apps/docs/integrations/per-page-markdown.mjs` and reuses the same patched
   Markdown converter
