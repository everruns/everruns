...
> `SessionFileSystemAdapter` is implemented in `crates/core/src/capabilities/bashkit_shell/mod.rs`.
>
...
The implementation is in `crates/core/src/capabilities/bashkit_shell/mod.rs` with `SessionFileSystemAdapter`.
...
See `crates/core/src/capabilities/bashkit_shell/mod.rs` for the full implementation.
...
`bashkit_shell` installs observational-only bashkit interceptors on every `Bash` instance it builds (`install_observability_hooks` in `crates/core/src/capabilities/bashkit_shell/mod.rs`). The hooks emit structured `tracing` events at the `bashkit.hook` target, tagged with the active `session_id` for audit correlation:
