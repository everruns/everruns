...
A **public endpoint** is an HTTP endpoint that accepts unauthenticated traffic and serves it from channel-owned, tenant-scoped configuration. Public endpoints have a different threat surface from authenticated management APIs: callers may be anonymous, may be hostile, and have no relationship to operators or owners. This spec defines the contract every public endpoint MUST satisfy.

## Current Public Endpoints

| Endpoint | Route | Source | Notes |
|---|---|---|---|
| AG-UI | `POST /v1/e/{channel_id}/ag-ui` | `crates/server/src/api/ag_ui.rs` | Public SSE stream; optional channel token |
| AG-UI image upload | `POST /v1/e/{channel_id}/ag-ui/images` | `crates/server/src/api/ag_ui.rs` | Public multipart image upload; optional channel token |
| FCP handshake | `GET /v1/e/{channel_id}/fcp` | `crates/server/src/api/fcp.rs` | Public Markdown handshake (always-open per FCP SPEC) |
| FCP message | `POST /v1/e/{channel_id}/fcp` | `crates/server/src/api/fcp.rs` | Public text-in / text-out; optional channel token; FCP-only rate limiter |
| Public Chat config | `GET /v1/e/{channel_id}/public-chat/config` | `crates/server/src/api/public_chat.rs` | Public non-secret bootstrap (branding, sign-in method, Turnstile site key) |
| Public Chat run | `POST /v1/e/{channel_id}/public-chat` | `crates/server/src/api/public_chat.rs` | Public AG-UI SSE stream; anonymous + optional Google sign-in; Turnstile for anonymous; own rate-limiter namespace |
| Slack events | `POST /v1/e/{channel_id}/slack/events` | `crates/server/src/api/slack_events/` | Anonymous Slack webhook (signature-verified) |
| Slack manifest | `GET /v1/e/{channel_id}/slack/manifest` | `crates/server/src/api/slack_events/` | Anonymous YAML manifest fetch |
...
The `/v1/e/{channel_id}/...` routes are canonical. Existing `/v1/apps/{app_id}/...` routes are permanent aliases. Alias resolution uses the channel's frozen `legacy_app_public_id`; traffic serving does not read `apps` or `app_channels`.
...
- **TM-AUTHZ-005**: live-channel + active-agent + `anonymous=true` gating before traffic reaches the handler.
- **TM-DOS-010**: per-org / per-session SSE connection limits via the shared SSE tracker.
- **TM-TENANT-009**: routing tags retain the legacy App and channel public IDs so compatibility sessions stay isolated.

Any new public endpoint MUST be reviewed against these mitigations before merge.

## Non-Goals

- The HTTP-level error responses returned *before* the streaming/payload phase (for example malformed input, a non-live channel, or `not_found()`) describe input shape or public endpoint availability. They keep their specific text when it is actionable and does not leak internal state. They are not routed through `PublicError`.
