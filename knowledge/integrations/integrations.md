...
| TypeSafe | [`integrations/typesafe/SPEC.md`](../../integrations/typesafe/SPEC.md) | Typed decision (probability, single choice, graded score) via TypeSafe System One, as the `jev_decision` tool. Experimental (Dev only). One crate covers the vendor client, the capability, the connector, and the [Decisions Service](../operations/decisions-service.md) that backs guardrail `jev` checks. |
