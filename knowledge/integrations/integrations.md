...
| TypeSafe | [`integrations/typesafe/SPEC.md`](../../integrations/typesafe/SPEC.md) | Typed judgments (probability, single choice, graded score) via TypeSafe System One, as the `jev_evaluate` tool. Experimental (Dev only). One crate covers the vendor client, the capability, the connector, and the [Classifier Service](../operations/classifier-service.md) that backs guardrail `jev` checks. |
