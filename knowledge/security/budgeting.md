...
**Subject types**: `session`, `agent_channel`, `app_channel`, `app`, `agent`, `user`, `org`, budgets cascade through the hierarchy from most specific (session) to most general (org). A session's effective budgets include all matching levels.

`agent_channel` budgets resolve from the session's structural channel reference (`sessions.channel_id`). The subject was named `agent_endpoint` before the Endpoint→Channel rename and that value is still accepted on input (see [agent-exposure.md](../integrations/agent-exposure.md#naming-endpoint-became-channel)). Slack and FCP routing tags do not determine budget identity. `app` and `app_channel` are legacy compatibility levels gated behind the experimental `app_budgets` feature flag (`FEATURE_APP_BUDGETS`, auto-enabled in `DeploymentGrade::Dev`). Sessions opt into those legacy levels via the standard tags emitted by the apps domain (`app:<app_id>`, `app_channel:<channel_id>`). The legacy `slack:app:<id>` tag is also recognised for backwards compatibility.
...
Look up session → find active budgets in hierarchy
  (root session → agent channel → app_channel → app → agent → user → org)
  │
