...
4. **Cache Creation Tokens**: Tokens written to cache (including Anthropic and newer OpenAI models)
...
before recording `input_tokens`. OpenAI Responses also subtracts cache writes
when reported; both typed and generic streaming parsers preserve these writes.
Anthropic/Bedrock already report disjoint
...
model's cheaper cache-read rate, and cache creations at the cache-write rate
