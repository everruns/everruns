...
| TM-DOS-040 | Prefix-accumulating live deltas exhaust framework observer memory | Medium | The framework's fixed-count event ring omits the redundant `accumulated` prefix from live `output.message.delta` JSON and retains only incremental chunks; app-listener queues contain only this prefix-free facade event, while host observability queues independently contain canonical core events after applying their filters. Completed output events still carry the final message. Buffered streaming text for a slow app listener therefore grows linearly rather than quadratically. | MITIGATED |
