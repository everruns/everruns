...
| Slack webhook forgery | TM-SLACK-001 | HMAC-SHA256 signing secret verification, 5-min replay window; a channel whose secret is not yet configured answers only Slack's `url_verification` handshake and rejects every other event with 401, and an empty secret is refused by the verifier itself rather than keying an HMAC anyone can compute |
