...
`everruns-host` additionally offers a real-process shell to embedders, behind Cargo features the server does not enable: `HostCompute` (TM-BASH-018) and the `host_shell` capability. There the isolation is the kernel's, Seatbelt on macOS and Landlock plus seccomp on Linux, and the relevant questions are whether that boundary can fail open, be widened by the model, or be escalated past without a person. TM-BASH-019 through TM-BASH-025 are about that path. TM-BASH-026 covers the Foreman example's independent test runner.
