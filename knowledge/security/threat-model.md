...
| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-MCP-001 | Everruns MCP server tenant escape | Critical | The first-party `/mcp` endpoint is mounted only when the deployment enables `FEATURE_MCP_ENDPOINT`. MCP-specific authentication and org resolution run before dispatch; both the selected organization and target-org overrides must have an effective `mcp_endpoint` opt-in, and overrides re-check current membership. `query` exposes only read-only inventory commands; `execute` and tier-1 agent/session tools dispatch through org-scoped domain commands and `Command::run` policy checks. The root-route API rate limiter remains applied to `/mcp`. Regression coverage verifies disabled organizations and non-opted-in override targets fail closed. | MITIGATED |
