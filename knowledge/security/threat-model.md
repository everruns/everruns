...
reasons: host environment configuration, disabled by default, model fixed in
code, reachable only through capability execution context. It is a second
service rather than a mode of the first because its answers are values, not
text — which is what removes the guardrail fail-open on an unparseable
verdict.
