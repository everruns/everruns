...
| TM-AGENT-015 | Dangling or concurrently resolved tool calls cause LLM confusion | Low | User messages, client results, URL consent, and timeouts compete through one atomic parked-turn claim. Only the claim holder can persist while competing callers receive a conflict. A user message cancels pending calls before resuming, durable resume is awaited before the session becomes active, and event or durable-enqueue failures restore the waiting state for retry. Transcript repair synthesizes any other missing results before an LLM call. | MITIGATED |
