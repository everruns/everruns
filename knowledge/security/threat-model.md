...
| TM-DURABLE-012 | Process-local code smuggled into a distributed Agent definition | High | `ScaleEngine` rejects every raw `everruns::Agent` before writes. Versioned `PortableAgent` contains stable provider/workspace/tool/capability/hook paths only; custom deserialization revalidates every path, and missing or duplicate Registry resolution fails closed with typed component/path errors without replacing an earlier registration. Closures, drivers, sinks, and code implementations have no portable field. | MITIGATED |
| TM-DURABLE-013 | Duplicate canonical events during concurrent append or restart | High | Scale owns one PostgreSQL canonical event log. A transaction-scoped per-session advisory lock serializes sequence assignment and insert; `(session_id, sequence)` and `event_id` are unique. Durable scheduling events never write the conversation transcript. Restart conformance asserts identity and sequence uniqueness. | MITIGATED |
| TM-DURABLE-014 | Workspace substitution or isolated-head reuse after worker restart | High | Scale persists the opaque Environment binding and requires the registered workspace implementation to reopen the exact provider/workspace/head tuple. Transaction-scoped head locking plus isolated-claim checks reject incompatible cross-session binding; resume never substitutes the new Agent's default head. | MITIGATED |
| TM-DURABLE-015 | Portable definition or registry path resource exhaustion | Medium | Registration paths are bounded to 200 bytes and 64-byte segments with a restricted ASCII alphabet; names/models, instructions, and total executable references are bounded before Registry lookup or persistence. | MITIGATED |
...
| TM-DOS-035 | Framework history cursor or lifecycle-heavy replay drives unbounded allocation/work | Medium | Public history is explicit bounded paging (100 messages by default, 256 maximum); cursors are opaque, session-bound snapshot tokens capped at 4 KiB before decoding; each host projection caps examined canonical envelopes at 100 000 and returns a typed `HistoryTooLarge` error rather than scanning indefinitely. Lazy whole-history traversal retains only one page at a time. | MITIGATED |
