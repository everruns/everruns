...
| TM-TOOL-031 | Repository-controlled MCP stdio configuration executes commands on the coding CLI host | High | The coding CLI never discovers `.mcp.json` from the selected repository. MCP servers, including process-spawning stdio servers, are registered only when the user explicitly supplies a trusted configuration with `--mcp-config`. A CLI regression test verifies repository-local configuration is ignored without that opt-in. | MITIGATED |
