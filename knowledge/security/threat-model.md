...
| TM-AUTH-020 | Public Agent endpoint auth bypass | High | AG-UI and A2A expose first-class endpoint auth with Google/OIDC JWT bearer, OAuth2 introspection, HTTP Basic, or trusted-header mTLS policy. Both ingress handlers resolve Agent endpoint liveness first, then call the shared `AppEndpointAuthVerifier` before session lookup, image upload, task polling, cancellation, or message dispatch. Missing, invalid, or corrupt credentials return generic failures and do not expose provider details. Legacy nested `channel_config.auth` is read only for compatibility and fails closed when malformed. Legacy AG-UI token and A2A API-key behavior remains the default only when both auth fields are absent. | MITIGATED |
...
| TM-AUTH-022 | JWKS / OIDC discovery abuse or poisoning | High | First-class OIDC auth validates issuer/JWKS/introspection URLs with `validate_safe_url`, rejects symmetric JWT algorithms for OIDC, requires issuer + audience + exp claims, validates `nbf`, and caches discovery/JWKS for a bounded 15 minutes. Provider fetch failures fail closed before session creation. | MITIGATED |
...
| TM-LLM-034 | Provider reasoning artifacts exposed through APIs, or chain-of-thought retained inconsistently | Medium | Reasoning is modelled as ordered `ContentPart::Reasoning` artifacts (`crates/provider/src/reasoning.rs`) that separate readable text from opaque replay state (provider signature, encrypted reasoning context). Replay state is carried verbatim for the driver to hand back and never leaves that boundary: `ReasoningContentPart::to_public` / `Message::into_public` strip it at the REST projection, `EventData::into_public` applies the same projection at the events read boundary so `GET /v1/sessions/{id}/events` and the SSE stream cannot republish what the message endpoint withholds (EVE-933), the `reason.item` event carries identity and provider-curated summary only (guarded by `test_reason_item_data_excludes_plaintext_reasoning`, which now also rejects `encrypted_content` / `signature` keys), ATIF exports emit readable reasoning as `reasoning_content` and skip the artifact, and driver logs record lengths and booleans rather than payloads. This narrows a prior surface: the published `Message` schema carried `thinking_signature`, and `reason.item` persisted `encrypted_content`. Readable chain-of-thought is still retained where the provider exposes it (`reason.thinking.completed` and the reasoning parts), and that retention is now uniform across providers rather than Anthropic-only — Chat Completions reasoning (DeepSeek-R1, Qwen, Groq) was previously parsed nowhere and silently discarded, so sessions on those models now persist reasoning they did not before. Operators who must not retain chain-of-thought should treat reasoning parts and `reason.thinking.*` as session content subject to the same retention controls as messages. | MITIGATED |
| TM-LLM-035 | Workspace instruction files (AGENTS.md hierarchy) steer the agent or poison the cache-stable system prefix | Medium | Resolved hierarchically from the filesystem root to the working directory (`crates/builtins/src/agent_instructions.rs`): names pass the same traversal guard as root-level resolution (`..` rejected), missing/blank files are skipped, and reads are bounded (16 files, 32 KiB each, 128 KiB total per turn with an explicit omission note). Rendered as the leading user-role message behind a trust framing header in `<agent-instructions source="...">` sections — never the system prompt — so file content cannot share privilege with harness safety instructions or invalidate the cache-stable system prefix (same posture as TM-TOOL-010 skill content). Collected per turn through `conversation_context_contribution` and inserted at the head of the per-request messages in `crates/engine/src/execution/reason.rs`. Residual risk is the model following a malicious file despite the framing, an inherent LLM limitation (cf. TM-LLM-007). | MITIGATED |
...
| TM-TENANT-012 | Latent cross-org regression via bare-id repository methods returning secrets/content | Low | Defense-in-depth; **no exploitable path today**. Agent endpoint management reads and writes are scoped by org and Agent. Slack's bare-id metadata update runs only after ingress resolves the endpoint and its owning Agent; it writes the resolved row directly and accepts no caller-supplied endpoint id. Remaining production bare-id methods cover `agent_identity_connections`; every caller first resolves the owning identity under the caller's org, while service-grant and runtime paths bind the exact active Agent, identity, and org before reading or writing credentials. The removed `get_session_file_by_id` leaves no bare-id file-content reader. A future caller that skips ownership resolution would reintroduce the hazard, so the remaining methods retain source invariants. (EVE-634) | MITIGATED (defense-in-depth) |
...
| TM-TENANT-015 | Harness shared memory widens a note to the whole org | Medium | Platform Chat v2 mounts one org-scoped Memory at `/memory/shared` that every session of that harness reads and writes, so a note one member leaves is readable by every other member of the org who can open that surface. Deliberate, and the one place `/memory` is not private: `/memory/agent` and `/memory/user` stay scoped by owner, and `/memory/user` additionally goes through the `resolved_owner_user_id` check in `session_files::queries::verify_session` (TM-TENANT-013), which this change does not touch. Which harnesses get one is a source-level allowlist (`shared_memory_name_for_harness`), not configuration, so widening the blast radius is a code review rather than a setting; the harness itself is org-opt-in behind the `platform_chat_v2` feature flag, and its prompt states that sharing is not reversible. Cross-org access is unaffected: the Memory is keyed by `UNIQUE(org_id, name)` and every read resolves through `get_memory_by_id(mount.org_id, …)`. | ACCEPTED (by design) |
...
| TM-AUTHZ-004 | Missing policy on mutating command | Medium | Every caller (HTTP/MCP/gRPC/platform) routes through `Command::run` which evaluates `Command::policy()`. Inventory coverage test (`crates/server/tests/domain/command_policy_enforcement_test.rs`) asserts every non-GET command declares a policy, a missing declaration fails the build | MITIGATED |
| TM-AUTHZ-005 | Anonymous endpoint reaches draft, disabled, or protected endpoint config | High | Public AG-UI ingress resolves `agent_endpoints JOIN agents`, then requires endpoint status `live`, endpoint enabled, Agent active, and Agent exposures not suspended before request work. Legacy configs then require `anonymous=true` plus the configured token when present. First-class endpoint auth bypasses the legacy anonymous flag only after the shared verifier accepts the configured credential policy. Nested `channel_config.auth` remains a read-only compatibility source and malformed stored policy fails closed. Failures return before session creation or image upload. | MITIGATED |
| TM-AUTHZ-006 | Anonymous webhook reaches a non-live endpoint | High | Public webhook ingress resolves liveness through `app_ingress::endpoint_liveness` — endpoint `status = live`, agent `active`, exposures not suspended — before the per-channel token is checked, and only then creates or reuses a session. Liveness precedes auth so a caller cannot distinguish a misconfigured endpoint from a bad token, and every not-live reason collapses to the same generic rejection. Replaced the `App.status × channel.enabled` gate in EVE-1007. | MITIGATED |
| TM-AUTHZ-007 | HTTP callers bypass declared command policy | High | Before the command runner, HTTP adapters called `Command::execute` directly, skipping `Command::policy()`. Now all adapters call `Command::run`, which enforces policy using `ctx.permission_resolver.evaluate_with`. One handler still answers before the dispatcher: MCP form-mode elicitation for `ask_user` (`api/mcp_endpoint/form_elicitation.rs`) reads a session's pending questions and applies answers itself, so it evaluates `SESSION_VIEW` before the read and `SESSION_MANAGE` before the write, against the same `AuthState::permission_resolver` the dispatcher uses. A pre-dispatcher handler added later owes the same two checks. Tests: `run_blocks_member_from_manage_command`, `dispatch_blocks_member_from_manage_command`, `session_policy_denial_does_not_disclose_or_resolve_a_question_set` | MITIGATED |
...
| TM-AUTHZ-009 | External caller spoofs `app:`/`app_channel:` session tag to attach to another app's budget | Medium | App-scoped budgets cascade onto sessions via `app:<id>` / `app_channel:<id>` tags (see knowledge/security/budgeting.md). `SessionService::create` now rejects these prefixes from non-internal callers, mirroring the existing `__internal:` reservation. Only the apps domain (which routes through `Caller::internal`) can stamp them, so an org member cannot opt their personal session into a sibling app's budget cap. EVE-1004 appends the per-transport endpoint namespaces `slack:endpoint:` and `fcp:endpoint:`, which migration 137 reads to attribute a session to the endpoint it arrived through. Runtime endpoint budget binding does not parse these tags; it resolves the session's structural `endpoint_id` through an org-scoped endpoint ownership query. The list is append-only: every routing and attribution lookup also anchors on `app_id`, which only internal callers set, so a forged tag was never sufficient on its own — the reservation is what keeps that true as readers are added. | MITIGATED |
| TM-AUTHZ-010 | Disabled LLM model still reachable through resolution paths | High | `llm_models.enabled = FALSE` is enforced at every model-resolution read: `Database::get_default_llm_model`, `get_llm_model_by_model_id`, and `get_llm_model` (UUID lookup used by agent execution and validation) all add `AND m.enabled = TRUE`; the in-memory backend mirrors the same gate. Admin listing (`list_all_llm_models`) intentionally returns disabled rows so operators can re-enable them through the management UI. Admin mutations use a dedicated unfiltered, org-scoped model read, then resolve the provider in the same org, so disabled rows stay manageable and a missing or foreign provider link cannot bypass managed-catalog policy. Tests: `test_disabled_model_is_not_resolvable_or_default_postgres` (`crates/server/tests/server_integration/repository_integration_test.rs`) and `model_with_foreign_provider_link_cannot_be_deleted` (`crates/server/src/domains/models/service.rs`) | MITIGATED |
...
| TM-AUTHZ-018 | A user creates or replaces a service-owned MCP grant without authority over the MCP server or agent identity | High | Identity-mode authorize requires `MCP_SERVER_MANAGE` before discovery or identity creation. The browser-bound pending state carries the exact resolved agent and identity ids. The callback re-evaluates the active permission resolver and atomically locks and verifies that both records remain active, linked, and under the authenticated org before storing an encrypted grant. Permission denial, discovery/configuration failure, callback denial, archived-server, state-mismatch, and callback-after-agent-deletion tests fail before grant persistence. | MITIGATED |
| TM-AUTHZ-019 | A retained App-shaped ingress alias bypasses Agent endpoint ownership or liveness | High | Every permanent `/v1/apps/{app_id}/...` alias resolves through `agent_endpoints.legacy_app_public_id` and `agents`; traffic-serving code does not read `apps` or the `app_channels` compatibility view. Alias and canonical endpoint routes share the endpoint-owned authentication, tenant, owner-principal, version, and liveness context. A DB-backed HTTP regression exercises every retained alias under a PostgreSQL role that is denied reads from both frozen App relations. | MITIGATED |
...
| TM-API-005 | Internal or database error detail leakage | Medium | The HTTP command dispatcher, MCP scripted command adapter, and gRPC `ExecuteCommand` adapter return generic details for internal errors. Typed PostgreSQL unique violations keep `409` and `already_exists` while replacing constraint and source-location text with `Resource already exists`; intentional domain conflict messages remain intact. Full diagnostics stay in server logs. Public command-path tests cover all three adapters. The direct Worker gRPC RPCs are held to the same bar (EVE-996): every internal failure in `worker_service_impl.rs` goes through `internal_status`, which logs the source error and returns a fixed message, because workers copy `status.message()` verbatim into runtime errors (`grpc_status_to_error`) and from there into worker logs and durable failure records. A source-shape test in the same file fails if a new RPC formats its error into the `Status`. | MITIGATED |
...
| TM-API-013 | LLM provider base URL SSRF | High | `validate_safe_url` blocks private IPs, loopback, link-local, cloud metadata, non-HTTPS on provider create/update (EVE-69). Request-time the shared provider HTTP clients (`driver_helpers::shared_streaming_http_client` / `shared_request_http_client`, used by the OpenAI-protocol/Anthropic/Gemini/embeddings drivers) disable redirect following and install an SSRF-guarding DNS resolver that rejects any hostname resolving to a blocked range, closing the DNS-rebind/302-redirect gap a create-time-only check leaves open (EVE-623). The public OpenAI-compatible model-discovery helper also validates the URL before resolving authentication, covering IP literals that bypass the DNS resolver even when called outside provider registration. `POST /v1/providers/check-credentials` (TM-LLM-036) probes an unsaved credential and is a second entry point to the same egress, so it runs the identical `validate_provider_base_url` / `validate_safe_url` gate before any request | MITIGATED |
...
| TM-API-023 | Git source redirect SSRF | High | Memory and Knowledge Index git source URLs are statically restricted to public HTTPS targets before storage. The smart-HTTP client additionally disables redirects, so an attacker-controlled public repository endpoint cannot redirect discovery GETs or upload-pack POSTs to loopback, link-local, private-network, metadata, or downgraded HTTP targets. Redirect responses fail as unreachable remotes. See `crates/server/src/domains/git_fetch.rs`. | MITIGATED |
| TM-API-024 | Elicitation page completed by the wrong person (spec phishing case) | High | Pages under `/mcp/elicitations/*` serve URL mode elicitations Everruns raises over `/mcp`. The signed token names what to collect and for whom and grants nothing on its own: every page additionally requires the visitor's own browser session and refuses unless that session is the principal the elicitation was minted for, which is what stops a handed-off link from binding a credential to the wrong identity. Tokens are HMAC-signed with a 15-minute expiry, re-verified on submit, and org membership is re-read from the database before anything is stored. Pages are `no-store`, `frame-ancestors 'none'`, `no-referrer`, and script-free; the form posts the value straight back to Everruns, so it never passes through the MCP client or model context. | MITIGATED |
| TM-API-025 | Provider discovery credentials sent to a fallback origin | High | Model-list URL derivation changes only the configured URL path, preserving origin and query parameters instead of falling back to another provider. Invalid URLs remain invalid. Public driver authentication probes verify the derived destination before any network request. See `models_url_for_api_url` in `crates/provider/src/openai_protocol.rs`. | MITIGATED |
...
| TM-TOOL-024 | WebFetch rendered-page JavaScript escapes egress controls or exhausts worker resources | High | Not applicable: in-process rendering is not compiled in. FetchKit's `render-rakers` feature is disabled, so the tool never advertises a `render` argument and a request for one is refused with `RenderNotAvailable`. JavaScript-rendered pages are served by the browserless and deno integrations, which carry their own sandbox controls. | MITIGATED |
| TM-TOOL-025 | MCP OAuth refresh-token theft, torn rotation, or refresh stampede | High | Refresh tokens stay envelope-encrypted and are never logged; token endpoints are revalidated and DNS-pinned through `EgressService` for every refresh (TM-TOOL-018); a bounded per-grant single-flight coalesces concurrent refreshes; access token, rotated refresh token, expiry, and scope are persisted atomically before the new access token is returned. A provider `invalid_grant` response revokes the unusable service grant, while transient provider failures preserve it for a later retry. Rejected or incomplete grants fail closed to `connection_required`. | MITIGATED |
...
| TM-TOOL-032 | MCP credentials leak through serialized execution snapshots | High | The credential-free MCP scope projection omits connection URLs, stdio commands, and arguments because each can contain inline credentials. It retains only non-secret connection metadata plus header and environment variable names; the host resolves the live connection separately. Sentinel tests cover serialization and `Debug` output. | MITIGATED |
...
| TM-TOOL-033 | Hostile elicitation URL reaches a consent surface, or is fetched by the client | High | A URL mode elicitation is a third-party server asking that a person open a link. `validate_elicitation_url` (`crates/mcp/src/elicitation.rs`) accepts `https` only, with loopback `http` for local development, before any handler sees the URL, so `javascript:`, `file:` and `data:` can never reach a consent surface. The client never fetches the URL or its metadata; it passes the host and a Punycode flag alongside so the surface highlights the domain and warns on internationalized labels, per the spec's subdomain-spoofing guidance. | MITIGATED |
| TM-TOOL-034 | Recorded elicitation consent replayed, or reused for a domain the user never saw | High | Consent is a single-use `StoredConsent` in session storage naming the server, tool, host and expiry (30 min). `SessionElicitationConsents::take_consent` deletes the record before honouring it, so one consent authorises exactly one `accept`; `StoredConsent::grant_for` re-checks the server/tool pair (a folded storage key cannot grant the wrong pairing) and `ConsentingUrlElicitations` compares the consented host against the host the server elicits on the retry. A server that elicits one domain, waits for the click, then elicits another gets no reuse: the user is asked again. An unreadable store never becomes an implicit yes. | MITIGATED |
| TM-TOOL-035 | Client records consent for an elicitation the user was never shown | High | `POST /v1/sessions/{id}/mcp-elicitation-consent` accepts only a tool call id and accept/decline. The server, tool and domain the consent is written against are read from the `confirm_url_elicitation` event Everruns itself emitted (`crates/server/src/api/mcp_url_consent.rs`), never from the request body, and the route requires the session to be in `waiting_for_tool_results` and resolves the caller's org like every other session route. | MITIGATED |
| TM-TOOL-036 | Truncated or rejected pending model tool calls become executable after a terminal delta | Medium | Chat Completions captures the terminal reason before processing content or tool fragments, so its final fallback discards pending calls after length or content-filter termination. HTTP stream regressions cover a complete JSON tool fragment sharing the terminal chunk and verify no tool calls escape to the runtime. See `crates/provider/src/openai_protocol.rs`. | MITIGATED |
| TM-TOOL-037 | Truncated or rejected Gemini tool calls released after a terminal frame | Medium | Gemini processes all parts and the terminal reason before yielding a frame's events. Only accepted termination releases pending calls; truncation and safety rejection discard them. Later usage is retained without reopening content. Complete HTTP stream fixtures cover multiple calls, same-frame termination and rejected calls in `crates/drivers/gemini/`. | MITIGATED |
| TM-TOOL-038 | Gemini adapters corrupt tool data while translating schemas or streamed arguments | Medium | Schema cleanup visits only schema-valued keywords, preserving literal defaults, enums, examples and extension payloads. The shared incremental UTF-8/SSE decoder assembles transport fragments before Gemini JSON parsing. Exact schema fixtures and every-byte-boundary stream tests preserve both answer text and tool arguments in `crates/drivers/gemini/src/driver.rs`. | MITIGATED |
| TM-TOOL-039 | Bedrock tool identifiers or schema bounds change through floating-point rounding | Medium | The Bedrock JSON-to-document adapter preserves unsigned integers as integer documents before considering floating-point conversion. Complete nested tool-argument tests cover values above the signed range and the unsigned maximum, alongside scalar types and message ordering. See `crates/drivers/bedrock/src/driver.rs`. | MITIGATED |
| TM-TOOL-040 | Native asynchronous calls bypass execution policy, replay under stale worker ownership, or finish a turn before outputs are delivered | High | Explicit tool selection, ordinary Act authorization/hooks/scoping, and read-only async eligibility gate dispatch. File-saving lookups and finalized-output hook combinations are rejected. Tenant/session/turn-scoped database-clock leases fence encrypted, size-bounded checkpoints; ownership loss aborts local jobs. Original call IDs retain terminal tombstones, HTTP delivery intent and transcript boundaries fail closed when ambiguous, and the scheduler receives completion only after every output has a provider receipt. See [native async execution](../execution/native-async-tools.md). | MITIGATED |
| TM-TOOL-041 | An MCP attachment resolves a credential belonging to a different identity than its `actsAs` declares, so a write lands under the wrong remote account with no configuration change and no event recording the substitution | High | MCP credential resolution is a pure function of the attachment's `actsAs`, enforced in the resolver rather than at validation so it also holds for configs written before validation existed or written straight to the database. `none` reads no connection store, `service` reads only `agent_identity_connections`, `user` reads only `user_connections` for the invoking user; no arm falls back to another. A `user` attachment additionally has the preset's `api_key` and any preset or inline `Authorization` header discarded, so org-held service auth cannot present as the user. The identity-preferring lookup in `get_connection_token` remains only for the non-MCP providers that still depend on it. See `crates/server/src/storage/connection_resolver.rs` and `crates/server/src/domains/mcp_servers/scoped_mcp.rs`. | MITIGATED |
| TM-TOOL-042 | An unattended run borrows a human's MCP credential, spending a person's token for work no person initiated (first-org-member-wins) | High | A `user` attachment in a session with no human initiator fails closed to `connection_required` instead of resolving the owner's token. Attendedness is decided by the session's owning principal being itself a `user` principal, not by the denormalized `sessions.resolved_owner_user_id`, which walks the principal parent chain and therefore still reports a human for a trigger-created agent-identity principal. See `session_has_human_initiator` in `crates/server/src/storage/repositories/user_connections.rs`. | MITIGATED |
...
| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-MCP-001 | Everruns MCP server tenant escape | Critical | The first-party `/mcp` endpoint is always mounted, so feature availability is deliberately not a security boundary. MCP-specific authentication and org resolution run before dispatch; target-org overrides re-check current membership; `query` exposes only read-only inventory commands; `execute` and tier-1 agent/session tools dispatch through org-scoped domain commands and `Command::run` policy checks. The root-route API rate limiter remains applied to `/mcp`. Authenticated smoke testing verifies anonymous requests fail closed; regression coverage in `crates/server/tests/domain/mcp_endpoint_test.rs` proves fresh organizations need no opt-in and chains `discover`, `query`, `execute`, `agent_run`, `session_get_status`, and `session_send_message` against cross-org bait with no read/write escape; resources/read is also covered against cross-org bait. | MITIGATED |
| TM-MCP-002 | Mutating command exposed through read-only `query` | High | `query` builds a read-only command toolset from `Command::read_only()`. Inventory coverage in `crates/server/tests/domain/command_policy_enforcement_test.rs` allows only a small reviewed set of POST-style read helpers to override `read_only() == true`. | MITIGATED |
...
| TM-LLM-021 | Utility LLM key exposed through agent model configuration | High | The utility LLM uses a deployment env secret (`UTILITY_OPENAI_API_KEY`, or `UTILITY_OPENROUTER_API_KEY` when the deployment routes it through OpenRouter), is carried as a host service on `HostComposition`, and is threaded only into capability `ToolContext`. It is not stored in provider records, exposed through model selection, or accepted from session/agent config. Backend and model (`UTILITY_LLM_MODEL`) are resolved once at startup from the process environment; `UtilityLlmRequest` carries no model or credential field, so neither is reachable from agent, session, or API input. | MITIGATED |
| TM-LLM-022 | Tenant execution silently spending platform env keys | High | `LlmResolverService::resolve_provider_api_key` and `resolve_provider_credentials` are fail-closed: they return `None` when no database key is found rather than falling back to `DEFAULT_*_API_KEY` env vars. A selected-but-unconfigured provider may still assemble a command context so setup can repair it, but `DriverRegistry` wraps its driver in a credential gate that rejects chat, model-listing, and compaction locally before network I/O. `ProviderStore::get_provider_config` has no implicit default, forcing custom hosts to declare credential ownership. Env var helpers remain available only for explicit dev/CLI entrypoints: each driver crate exposes a `from_env(id)`, and `EnvCredentialProvider` is the single component in the workspace that pairs a driver's declared variable names with a real `std::env::var` lookup. Drivers only *declare* names (`FormField::env`, `DriverDescriptor::base_url_env`, each following that vendor's own SDK); declaring reads nothing and is inert on the server, where the declaration is never consulted. The boundary is enforced at build time rather than by convention: `scripts/lib/check-provider-isolation.sh` fails if `crates/server/src` or `crates/platform/src` constructs an `EnvCredentialProvider` or calls a `from_env`/`provider_from_env`, and if a driver reads a credential variable itself. A driver that declares no variable can never be configured from the environment, whatever its id is spelled — the registry's default schema declares none, so env configurability is opt-in per driver. Resolution is all-or-nothing against the declared schema, so a partially populated environment (`AWS_REGION` with no AWS keys, a half-filled Entra block) yields no credential rather than a half-configured provider. For single-tenant/dev convenience, `seed::seed_default_provider_keys_from_env` may materialize `DEFAULT_*_API_KEY` into the **default org's** provider rows at startup (encrypted), gated by `SEED_DEFAULT_PROVIDER_KEYS_FROM_ENV` (defaults to `DeploymentGrade::is_dev()`). Non-dev opt-in is ignored while built-in signup or built-in OAuth can self-provision users into `DEFAULT_ORG_ID`, so open-registration deployments cannot seed platform keys into an org that untrusted users can join and spend from. See `knowledge/foundations/llm-drivers.md` (Key Resolution Contract). | MITIGATED |
...
| TM-LLM-032 | Connection request options abused as an egress or credential-at-rest surface | Medium | A provider connection may carry custom HTTP headers plus a prompt-cache diagnostics opt-in (`settings.request_options`, `knowledge/foundations/providers.md`), settable only by an org admin with `provider.manage` on a non-managed connection. Bounded and validated server-side (`validate_request_options` in `crates/server/src/domains/providers/service.rs`): at most 16 headers, syntactically valid header names, values under 2 KB, and transport-owned names (`host`, `content-length`, `transfer-encoding`, `connection`, `upgrade`) rejected, so a header cannot repoint the SSRF-guarded connection at another virtual host (TM-API-013) or corrupt framing; drivers drop the same set again at merge time. Headers only decorate requests to the connection's already-validated base URL, they do not add a new egress destination. Values are stored in the `settings` JSONB **unencrypted**, unlike `credentials_encrypted` (TM-LLM-001), but provider API responses redact every value while retaining header names and non-secret diagnostics configuration: `provider.view` is a member-level permission and `provider.manage` is not, so an unredacted read would hand every org member a value only an admin could set. Writes fold a blank submitted value onto the stored one (`merge_request_options`), so an editor that round-trips a redacted read cannot silently erase a gateway credential. Diagnostics are an opt-in flag only; the provider's response payload is logged and carried verbatim without being interpreted. | **ACCEPTED** |
| TM-LLM-036 | Unstored credential check abused as an egress trigger or credential oracle | Medium | `POST /v1/providers/check-credentials` (`knowledge/foundations/providers.md`) probes a candidate provider key with a read-only `list_models` call before any provider row exists. It adds no capability an org admin lacks: `provider.manage` gates it exactly as create + `sync-models` do, the destination is driver-declared and any supplied `base_url` passes the same SSRF gate as create (TM-API-013), and the request rides the SSRF-guarded shared provider clients. The submitted credential is never persisted, never logged, and never echoed; failures are logged server-side and returned as fixed strings classified from `LlmErrorKind`, so the response distinguishes "key rejected" from "provider unreachable" without leaking the provider's response body (TM-LLM-003). Unauthenticated probing is impossible, and an authenticated admin could already drive the same outbound call by saving a provider and syncing models. The Settings provider forms call it on input rather than on a button press (`use-credential-check.ts`), so the same admin-authorized egress now fires from typing; a debounce plus stale-response drop bounds it to pauses in typing rather than keystrokes, and the caller, destination, and disclosure are unchanged. | MITIGATED |
| TM-LLM-037 | Decisions service key exposed through agent configuration, or confused with the agent-facing TypeSafe connection | High | The deployment decisions service (`knowledge/operations/decisions-service.md`) reads `UTILITY_TYPESAFE_API_KEY` from the process environment, is carried as a host service on `HostComposition`, and is threaded only into capability execution context (`ToolContext`, `PostGenerationOutputContext`). It is not stored in provider records, not exposed through model selection, and not accepted from session or agent config. The decision contract can name a model, but the platform's composition never sets one and no guardrail or agent config field reaches it, so the selectable knob is absent from every surface an agent can write. The agent-facing `jev` capability is a **separate** surface that resolves a user-scoped connection token (or a session secret), never the deployment key, so an agent cannot spend or read the platform credential. Both clients redact the key from `Debug`, and API error bodies are never surfaced verbatim because an upstream error can echo the request headers that carry it. | MITIGATED |
| TM-LLM-038 | Guardrail `jev` engine: content egress and verdict steering | Medium | A `llm_judge` or `moderation` check with `engine: "jev"` sends the same bounded stage excerpt (2 000 bytes at the tool seams, 4 000 at the output seam, UTF-8-safe) to the deployment's decisions service instead of the utility model: the same egress class, an operator-configured destination, no per-agent choice. The inspected content is passed as request **state**, never as instructions, and every question states that the content is data being inspected; the policy itself is operator-authored and lives in the question. Steering cannot manufacture a verdict, because there is no verdict to write: the model returns a probability and the configured `threshold` decides in code, which also removes the utility-LLM path's unparseable-verdict fail-open. Latency is bounded by one batched request per stage under a 10 s timeout, capped at `MAX_DECISIONS_QUESTIONS` (24) questions (TM-DOS); the deployment client does not retry. Any failure (timeout, error, unanswered question, no decisions service configured) fails open (`allow`), so a decisions-service outage can only ever block less, never make execution more permissive than the no-guardrail baseline. | MITIGATED |
| TM-LLM-039 | Captured request body widens where a prompt can be written | Medium | `LlmCallConfig::capture_request` records the driver's serialized request body on `LlmCompletionMetadata::request_body` for evaluation and request-log consumers that would otherwise reconstruct an approximation. The body carries the whole prompt, so the capture is **off by default** and is a per-call decision by the caller that owns the destination. It is the request body only: authentication travels in headers and is never serialized into it (asserted in `crates/provider/src/openai_protocol.rs` tests). Nothing in the runtime turns it on, and no API response or event carries the field today, so enabling it is what decides where the prompt lands. | **CALLER RISK** |
| TM-LLM-033 | Session transcript steers its own generated run summary, or the summary becomes an unbounded/uncosted LLM surface | Medium | Run summaries (EVE-867, `crates/server/src/services/run_summary.rs`) send run content to the **org-independent utility LLM**, the same footing as `AnalyzeAgent` (TM-LLM-030's provider reasoning), so no new vendor or cross-tenant egress is introduced and the spend is outside the session's budget and token counters. Transcript content is untrusted input: instructions live only in the system prompt and are never interpolated with run content, which is passed as a separate user message inside a labelled `RUN_DIGEST` fence with newlines flattened per event so no event can forge fence structure or address the model. The digest is bounded three ways — at most 120 trailing events, 400 bytes per event, 8 000 bytes total — and successful tool output (the bulkiest, least trustworthy content) is excluded entirely, only failures are carried. Output is bounded at 120 tokens and rejected above 400 characters, so a runaway generation is never stored. One call per terminal turn with a 30 s timeout and **no retry**: a failure costs a missing sentence, never a retry storm against a paid endpoint. Generation is spawned and awaited by nobody, so it cannot delay or fail a run, and it is skipped entirely for chat sources and when no utility LLM is configured. The stored value is rendered as text, never as markup. Residual risk is the summary sentence itself being attacker-influenced prose shown to an operator, which is why it is Medium rather than closed. | MITIGATED |
...
**TM-LLM-037, Decisions Service:**
The decision service has the same posture as the utility LLM and for the same
reasons: host environment configuration, disabled by default, reachable only
through capability execution context. Model choice differs in mechanism, not in
effect: the contract carries an optional model name so an embedding application
can pin a version, and the platform pins its own by never setting it and never
exposing a config field that could. The agent-writable surface has no model knob
either way. It is a second service rather than a mode of the first because its
answers are values, not text — which is what removes the guardrail fail-open on
...
| TM-OBS-007 | No security audit logging | Medium | Structured audit_logs table with fire-and-forget writes for auth events and for agent-domain events (app invocations, soft-approval asks and grants, see TM-OBS-011); admin-only query API | MITIGATED |
...
| TM-OBS-011 | Session conversation text widened to org admins through approval audit rows | Low | `soft_approval` grants are copied from the session event log into `audit_logs` by `ApprovalAuditListener` (`crates/server/src/services/approval_audit.rs`), which `AUDIT_LOG_VIEW` exposes to org admins who may hold no access to the session itself. The copied fields are model-authored from the conversation and can carry whatever a participant pasted; the tool descriptions instruct the model not to pass secrets, which is guidance, not enforcement. Each free-text detail is bounded to 512 characters with an explicit truncation marker (`MAX_DETAIL_CHARS`, marker TM-OBS-007), so the row carries an excerpt plus the turn/message correlation ids rather than the full text, and a model emitting a huge argument cannot write an unbounded row. The complete text remains only in the session event log under session permissions. Actor and `org_id` are resolved server-side from the session and its authenticated `input.message` initiator, never from tool arguments, so a model cannot forge either. | MITIGATED |
| TM-OBS-012 | Service-owned MCP activity is recorded as the invoking user or accepts a forged acting principal | Medium | Before durable tool-started and tool-completed events are written, the event service resolves the effective scoped MCP attachment and the session's agent-identity principal under the session org. For `actsAs: service`, it overwrites `acting_principal_id` with that principal and preserves a supplied human `initiator_principal_id`; when the initiator is absent it records the session owner. Missing principals and session, agent, configuration, or principal lookup failures prevent event persistence. Event metadata contains principal ids only, never OAuth credentials. | MITIGATED |
...
| TM-WEB-018 | Phishing link or transcript flooding via a provider-supplied attestation confirmation URL | Medium | `provider_attestation_required` (EVE-952) promotes a URL out of the provider's error body into prose the reader is told to visit, so a hostile or compromised provider endpoint — including a user-configured OpenAI-compatible base URL — chooses that destination. `ChatErrorAlert` renders the turn-failure path as plain auto-escaped React text (no markdown, no autolink); the agent-message path (`getLocalizedOutputMessageText`) goes through Streamdown, where the URL does become a link. `attestation_confirm_url` (`crates/provider/src/user_facing_error.rs`) accepts only `^https?://` (so `javascript:`/`data:`/`file:` can never become the link) and drops any URL over 300 characters rather than truncating it, falling back to OpenRouter's own settings page; gate types are capped at 8 entries of 64 characters each. The host is deliberately not pinned: the confirmation page is not always on the API host, and the residual risk is a link to an attacker-chosen HTTPS page, not script execution. `MarkdownLink` rejects non-http(s) schemes independently (mirrors TM-WEB-012), and the public surface never sees this code at all — `PublicError::from_internal_code` maps it to `service_unavailable`. | MITIGATED |
...
| TM-AGENT-015 | Dangling or concurrently resolved tool calls cause LLM confusion | Low | User messages, client results, URL consent, and timeouts compete through one atomic parked-turn claim. Only the claim holder can persist while competing callers receive a conflict. Expired claims are reclaimed only by the same resolver kind or by the recovery sweep, so a different action cannot acknowledge discarded input. Ask User answers bind only to the newest requested tool-call batch, so a stale card cannot resolve a later pause. A user message cancels pending calls before resuming, durable resume is awaited before the session becomes active, and event or durable-enqueue failures restore the waiting state for retry. Transcript repair synthesizes any other missing results before an LLM call. | MITIGATED |
| TM-AGENT-016 | Plaintext secrets in chat history | Medium | `ask_user` with `kind: "secret"` gives an agent an in-flow way to collect a credential that never reaches the event log: the client stores the value through the encrypted session-secret endpoint and the tool result carries only a `session:{name}` reference, with no `value` field on the answer type to put one in. Prompt guidance names it as the affordance instead of asking in prose. Does not stop a user pasting a key into an ordinary chat message; it removes the reason to | MITIGATED (partial, in-flow only) |
...
**TM-AGENT-016, Plaintext Secrets in Chat History (MITIGATED, partial, in-flow only):**
...
- **Current mitigations:** `ask_user` with `kind: "secret"` (EVE-1058) is the in-flow alternative that previously did not exist, which is what made "phase out in-chat secret collection" unfollowable. An `ask_user` answer is a *tool result*, so a credential typed into an ordinary question box would have been strictly worse than one typed in chat — plaintext in the event log **and** replayed into model context every turn. The secret kind is built so that cannot happen: `AskUserAnswer` has no `value` field at all (see `crates/builtins/src/ask_user.rs`), the client stores the value through the already-encrypted `PUT /v1/sessions/{id}/storage/secrets` path and then answers with a `session:{name}` reference, and `validate_answers` in `crates/server/src/api/question_answers.rs` refuses an answer that carries a selection or free text on a secret question rather than ignoring it. Free text and multi-select are normalized off the question for the same reason. The batch also never auto-resolves: a "default credential" is meaningless, so both `DefaultsResponder` and the engine's unattended path decline it, leaving "proceed without the credential" as the model's explicit decision.
  The other door is closed too (EVE-1059). An ordinary choice question with `allow_other: true` or a `text` question produces free text, and that text *is* a tool result, so the secret kind alone would have left `ask_user` opening this door wider than chat does. The deterministic credential-format check lives in `crates/server/src/credential_shape.rs`; `validate_answers` in `crates/server/src/api/question_answers.rs` invokes it over both paths **before** persistence and refuses a match — redacting afterwards is not an option, because there is no un-persisting an event. The rejection names the format and points at the secret flow, and never echoes the value into the error. The check is prefix-anchored rather than entropy-based on purpose: a missed credential is recoverable, but a false positive on a legitimate long opaque identifier is a dead end mid-conversation. Requiring a token boundary is what keeps `sk-` inside "ask-" from reading as a key, the same false positive that once reddened the hourly Actions-log sweep. That module is the single place the format list lives; it mirrors `scripts/scan_actions_log_secrets.py`.

- **Residual:** a user can still paste a key into an ordinary chat message and nothing here stops them. What this removes is the *reason* to — the agent now has an affordance to point at, and the prompt tells it to use that instead of asking in prose. Detecting secrets in arbitrary user messages is a different project. The format check also catches known formats only: a bespoke or unprefixed credential typed into a free-text box still gets through, which is the deliberate cost of not guessing at entropy.
- **Recommendation:** Settings > Connections remains right for a durable credential. Use the secret question for one a session needs in flow.
- **Priority:** High
...
- **Current mitigations:** (1) Per-layer `NetworkAccessList` (allowed/blocked patterns) on harness, agent, and session, merged narrow-only (intersection on `allowed`, union on `blocked`), with parsed-host checks for URL children and canonical URL-prefix comparison to prevent path/query suffix and dot-segment escapes, see `specs/network-access.md`; configurable via API and the agent/harness edit UI. (2) Optional deployment-wide system allowlist of curated public hosts, AND-ed as a hard ceiling, see `specs/system-allowlist.md`. (3) Both are enforced at the `EgressService` boundary; `web_fetch` routes through egress (`integrations/web-fetch/src/egress_transport.rs`) with the list re-checked on every redirect and crawl request. Direct-transport crawl is rejected while either policy is active because discovered pages cannot be re-checked there.
- **Current mitigations:** (1) Per-layer `NetworkAccessList` (allowed/blocked patterns) on harness, agent, and session, merged narrow-only (intersection on `allowed`, union on `blocked`), with parsed-host checks for URL children and canonical URL-prefix comparison to prevent path/query suffix and dot-segment escapes, see `knowledge/operations/network-access.md`; configurable via API and the agent/harness edit UI. (2) Optional deployment-wide system allowlist of curated public hosts, AND-ed as a hard ceiling, see `knowledge/operations/system-allowlist.md`. (3) Both are enforced at the `EgressService` boundary; `web_fetch` routes through egress (`integrations/web-fetch/src/egress_transport.rs`) with the list re-checked on every redirect hop.
...
`everruns-host` additionally offers a real-process shell to embedders, behind Cargo features the server does not enable: `HostCompute` (TM-BASH-018) and the `host_shell` capability. There the isolation is the kernel's, Seatbelt on macOS and Landlock plus seccomp on Linux, and the relevant questions are whether that boundary can fail open, be widened by the model, or be escalated past without a person. TM-BASH-019 through TM-BASH-026 are about that path. TM-BASH-027 covers the Foreman example's independent test runner, which is host execution reached from a repository rather than from a capability.
...
| TM-BASH-026 | Model-controlled working directory widens the native write boundary | Critical | The containment launcher passes the independently established workspace root separately from the requested working directory. The Linux worker opens the root and each cwd component with no-follow directory descriptors, rejects absolute/parent escapes, changes directory through the verified descriptor, and grants Landlock writes only beneath the root descriptor. Host compute rejects absolute and parent cwd values before launch. This prevents a workspace symlink or path swap from turning an external directory into the writable root. | MITIGATED |
| TM-BASH-027 | Sandboxed worker turns workspace edits into host code execution through Foreman tests | Critical | Foreman runs configured tests in a disposable Docker container. The repository is mounted read-only and copied to container-only tmpfs; the container receives no host environment or network, drops all capabilities, enables `no-new-privileges`, has a read-only root, and has CPU, memory, PID, output, and time limits. Timeout cleanup force-removes the container by its recorded ID. | MITIGATED |
...
| TM-DOS-004 | Database connection pool exhaustion | Medium | Two sqlx pools per instance with asymmetric acquire timeouts (EVE-1081): a request pool (`DATABASE_POOL_MAX`, short `DATABASE_ACQUIRE_TIMEOUT_SECS`) and a small reserved background pool (`DATABASE_BACKGROUND_POOL_MAX`, long `DATABASE_BACKGROUND_ACQUIRE_TIMEOUT_SECS`) that background sweeps draw from via `StorageBackend::for_background()`. Request acquires still wait for the configured timeout, then return a retryable HTTP 503 instead of a generic 500. Prometheus exposes each pool's current size, configured maximum, idle connections, and derived in-use connections. The split bounds the blast radius and the response makes overload explicit; it does not reject work before the acquire timeout or prevent a large burst from holding request sockets while it waits. | **PARTIAL** |
...
| TM-DOS-037 | Oversized hydrated capability transport | Medium | Agent and harness writes reject capability sets whose hydrated configuration exceeds the bounded worker transport budget, preventing valid declarative or plugin definitions from producing oversized durable-runtime gRPC responses. | MITIGATED |
| TM-LLM-040 | Provider key resolved against a host that only looks like the vendor, during model discovery | High | A vendor driver derives its listing URL from the org-configured base URL, so a lookalike (`api.openai.com.evil.example`), a userinfo spoof (`resource.openai.azure.com@evil.example`) or a path spoof (`evil.example/api.openai.com`) would receive the provider's credential if discovery ran against it. `supports_model_listing` (`crates/drivers/openai/src/driver.rs`) gates discovery to verified vendor hosts and returns `Ok(None)` otherwise, *before* `ProviderAuth::headers` is called. A generic OpenAI-compatible fallback is deliberately not attached to a vendor driver for this reason; callers that want one for a host they trust invoke `list_openai_compatible_models_best_effort` explicitly. Pinned by `public_discovery_gates_both_protocols_before_accessing_credentials`. | MITIGATED |
| TM-DOS-038 | Unbounded provider error body parsed to recover an error code | Low | A failed provider request's body is provider-controlled and nothing upstream bounds it. `provider_error_code_in` (`crates/provider/src/llm_error.rs`) refuses to parse a body over 64 KiB and caps the recovered code at 128 bytes, so neither the JSON parse nor the string retained on `LlmError` is sized by the provider. The surrounding message body stays as bounded as it already was. | MITIGATED |
| TM-DOS-039 | Unbounded provider stream exhausts a caller that folds it | Medium | A provider that streams without stopping had no per-call ceiling: the turn collector accumulated text until the process ran out. `TurnLimits` (`crates/provider/src/turn_collector.rs`) carries a whole-turn timeout, a first-event timeout, and a byte cap on accumulated answer text, enforced as the turn accumulates by both `collect_turn` and the `limit_stream` adapter, and set per call through `LlmCallConfig::limits`. Unbounded by default so no existing driver changes behavior; callers that need the ceiling opt in. | **CALLER RISK** |
...
| TM-DOS-032 | Provider outage traps a turn in layered retry loops | Medium | Provider request, first-stream reconnect, and reason-level recovery share strict attempt and elapsed-time budgets. Lower layers mark terminal retry decisions so an upper layer does not restart an already-exhausted policy. Backoff waits and retry attempts are charged to the elapsed budget; the request loop rechecks expiration after delayed wakeups before sending again. Provider reset durations and remaining-token headers reject overflow rather than wrapping; incomplete duration components are ignored. Exhaustion returns a resumable terminal failure instead of continuing indefinitely. | MITIGATED |
...
| TM-DOS-040 | Prefix-accumulating live deltas exhaust framework observer memory | Medium | The framework's fixed-count event ring omits the redundant `accumulated` prefix from live `output.message.delta` JSON and retains only incremental chunks; completed output events still carry the final message. Buffered streaming text therefore grows linearly rather than quadratically for a slow subscriber. A dispatch additionally builds one payload shared by every matching listener, so retained bytes do not scale with listener count, and clones the canonical core half only when a host observability listener matched it — an app-only build never retains a raw core event. A per-listener serialized-byte budget bounds what any one slow listener can pin regardless. | MITIGATED |
...
| TM-DOS-041 | A git source URL points at a remote that returns an unbounded packfile, exhausting worker memory during knowledge-index or memory sync | Medium | The fetch reads the response through a byte ceiling (`GIT_FETCH_MAX_PACK_BYTES`, default 512 MB; 32 MB for ref listings), rejecting a declared `Content-Length` over the limit before reading and refusing an over-long body rather than truncating it into a malformed pkt-line stream. Credentials are stripped by reqwest on cross-host redirects, and the resulting checkout is separately capped by the sync snapshot limits. See `crates/server/src/domains/git_fetch.rs`. | MITIGATED |
| TM-DOS-042 | Concurrent one-click Slack installs create orphan apps and exhaust a deployment-wide Slack quota | Medium | The install handler holds a transaction-scoped PostgreSQL advisory lock keyed by endpoint across app creation and persistence, then re-reads the endpoint after acquiring it. Concurrent requests therefore observe and reuse the winning app rather than issuing another external creation request. | MITIGATED |
...
| TM-A2A-008 | Stored API key material leaks through endpoint management reads | Medium | Agent endpoint and archival App reads redact `api_key_hash`; only the non-secret display prefix is returned. The retired App key-generation and key-rotation operations no longer expose plaintext | MITIGATED |
...
| TM-A2A-010 | Replay of captured request | Medium | Opt-in HMAC signing uses `X-Everruns-A2A-Timestamp` and `X-Everruns-A2A-Signature`. The server verifies HMAC-SHA256 over `v0:{timestamp}:{channel_scope}:{raw_body}`, where the scope retains the historical App and endpoint IDs for compatibility, and rejects cross-endpoint or out-of-window replay. The replay store is bounded by a five-minute TTL. Verification runs after primary authentication. The signing secret is encrypted at rest, redacted on read, and preserved across endpoint updates | MITIGATED |
...
| TM-A2A-012 | `tasks/get` / `tasks/cancel` cross-endpoint reads or destructive actions | Medium | Both handlers reuse the same auth, endpoint, and method gate as `message/send`, look up the session in the endpoint's org, and verify its permanent historical `app:` and `app_channel:` routing tags before returning or modifying anything. Credentials for one endpoint cannot access another endpoint's task. A failed binding returns `-32001 Task not found`. State derivation excludes prompts, tool arguments, and LLM output details | MITIGATED |
| TM-A2A-013 | DoS via runaway A2A client | Medium | Agent managers can configure a per-endpoint, per-IP request cap with `rate_limit_per_minute`. Enforcement runs after credential verification, uses the shared `ChannelRateLimiter` with a distinct `a2a` namespace, and scopes buckets by the permanent historical App and endpoint IDs. The server caps the field at `1_000_000`; `0` or absent falls back to the global API limit | MITIGATED |
| TM-A2A-014 | Agent Card advertises stale or wrong auth scheme | Medium | Agent Card security metadata is derived from the same effective endpoint auth used by request authentication. Nested legacy auth is read only for compatibility. Legacy endpoints advertise bearer API key; OIDC/Google emit `openIdConnect`, HTTP Basic emits `http/basic`, OAuth2 introspection emits generic HTTP bearer, and mTLS emits `mutualTLS`. The card is available only for live, enabled endpoints and never includes secrets | MITIGATED |

### Mitigation Details

**TM-A2A-001, API Key Generation:**
Existing keys retain the 32-byte random value created before App management retirement. The invocation path hashes the supplied value and compares it with the stored digest in constant time. Endpoint management and archival reads use the shared channel-config redaction path, which returns the non-secret prefix but removes the digest.

**TM-A2A-005, Method Gate:**
The handler dispatches only `message/send`, `message/stream`, `tasks/get`, and `tasks/cancel`, plus explicit legacy spellings for those four methods. Any other JSON-RPC method short-circuits with a structured `-32601` response before session work.

**TM-A2A-007, Tag-spoof Hardening:**
A2A reuses the frozen invocation compatibility runtime. Shared-session lookup requires the endpoint's resolved org, owner principal, and complete permanent routing tag set to match. A user-created session with spoofed tags is rejected.

## 23. FCP Channel (TM-FCP)

Agent endpoint-owned Free Communication Protocol ingress, with a permanent App-shaped route alias. Text-first HTTP endpoint with a deliberately minimal auth stack (anonymous + optional shared bearer token) and a dedicated `ChannelRateLimiter` namespace. Mitigations live in `crates/server/src/api/fcp.rs` and the Agent endpoint domain. See `knowledge/integrations/fcp-channel.md`.

| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-FCP-001 | Anonymous FCP request reaches a draft, disabled, suspended, or wrong-type endpoint | High | FCP resolution rejects an unknown endpoint, wrong transport, non-live or disabled endpoint, inactive Agent, or suspended Agent with the same generic 404 Markdown body. Caller cannot distinguish operator state, so the endpoint cannot be used as a probe oracle | MITIGATED |
| TM-FCP-002 | Timing oracle on token compare | Medium | `constant_time_eq` (`crates/server/src/api/fcp.rs`) bit-mixes the entire byte slice before returning, mirroring the A2A and AG-UI implementations. Token validation runs only after endpoint resolution, so timing differences for unknown endpoints are absorbed by the upstream 404 path | MITIGATED |
| TM-FCP-003 | Configured bearer token leaked in response body or logs | High | The 401 and handshake bodies are static templates that never interpolate the configured token. Agent endpoint and archival App reads redact the token and surface `token_configured: true`; storage keeps it only in encrypted `channel_config`. Tracing does not log the token | MITIGATED |
...
| TM-FCP-008 | Cross-org session reuse via FCP cookie spoofing | High | Endpoint resolution supplies the owning org and mandatory owner principal directly from `agent_endpoints JOIN agents`. Session reuse preserves the historical App attribution and routing tags, and still requires the resolved org, owner, and full tag set. A `fcp_session` cookie that does not match is ignored and a fresh endpoint-owned session is created | MITIGATED |
...
| TM-FCP-012 | Handshake (GET) used to probe operator existence | Low | The handshake is intentionally open per FCP SPEC, but resolution returns the same generic 404 body for unknown, draft, suspended, or wrong-type endpoints. The per-endpoint rate limiter also applies to GETs when configured | MITIGATED |

### Mitigation Details

**TM-FCP-003, Token redaction across read paths:**
The shared channel-config redaction path removes `token` and returns only `token_configured: true`. Agent endpoint updates merge preserved secret fields before encrypting the new configuration, so changing another field does not clear the token.

**TM-FCP-008, Session ownership invariant:**
The endpoint-owned execution context supplies the mandatory owner principal while retaining archival App attribution and the `fcp:app:<app_public_id>` tag for existing routes. The reuse path requires the resolved org, owner, and complete routing tag set to match, so guessing an alias or forging a cookie cannot adopt another endpoint's session.
...
| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-CI-001 | Workflow-level `DOPPLER_TOKEN` exfil via PR-controlled cargo build/test | Critical | `DOPPLER_TOKEN` is no longer declared at workflow `env:` in `ci.yml`, `brave-search-integration.yml`, `cursor-integration.yml`, or `sprites-integration.yml`. It is injected only into jobs/steps gated on `github.event_name == 'push'` (live-test jobs in ci.yml + per-integration live-test jobs). The Slack tests live in their own `slack-live-test` job (EVE-1039), whose job condition includes `github.event_name == 'push'` so the runner is never provisioned on a PR; within it `DOPPLER_TOKEN` is still injected via step-scoped env only, never at job `env:`; the `Run LLM integration tests` step injects it the same way inside `live-provider-matrix`, a job whose own condition is `push`-gated (EVE-938) | MITIGATED |
| TM-CI-002 | LLM provider keys (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY`) exfil from the live provider matrix's job env via PR-controlled cargo test | High | No provider keys are declared at any job `env:`. The single step that needs them (`Run LLM integration tests`) obtains them from Doppler at runtime via a step-scoped `DOPPLER_TOKEN` (`doppler run`), so the keys never enter a runner executing PR-controlled code. Since EVE-938 that step lives in its own `live-provider-matrix` job rather than inside `integration-test`; the job condition includes `github.event_name == 'push'`, which is a strictly stronger gate than the step-level `if:` it replaced — the runner is never provisioned at all on a PR | MITIGATED |
| TM-CI-003 | LLM provider keys exfil from PR-built `everruns-server`/`everruns-worker` binaries in `workflow-test` | High | No longer key-free: `workflow_test.rs` covers live provider paths (reasoning projection, Anthropic tool calls and extended thinking) that LlmSim cannot prove, so the job needs real credentials. Scoped instead of eliminated — no key at workflow or job `env:`; the two steps that need one (`Start API server`, `Run workflow tests`) inject a step-scoped `DOPPLER_TOKEN` and read the secrets from Doppler at runtime. `Start API server` fetches exactly `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` as `DEFAULT_*_API_KEY` for the server process only; the worker resolves credentials over the gRPC control-plane and never receives one. The enclosing job is `github.event_name == 'push'`-gated and `ci.yml` triggers `push` on `main` only, so the binaries running with keys are always post-merge code, never fork-PR-controlled. Compile-time PR coverage preserved via `build-binaries` | MITIGATED |
...
| TM-CI-009 | Runtime-fetched credential disclosed in a public workflow log | High | The Actions log masker only knows values that arrived through `secrets.*`. A credential fetched at runtime (`doppler secrets get`) is invisible to it, so writing one to `$GITHUB_ENV` or `$GITHUB_OUTPUT` publishes it in the `env:` group the runner prints for every later step — no `echo` required. Observed in run 35305647248, which put live TypeSafe keys in a public log. Fetched credentials are now scoped to the one process that needs them (`doppler run -- <cmd>`); `scripts/test-workflow-secret-handling.sh` fails CI on any workflow that writes a credential to a log-visible surface, and hourly `actions-log-scan.yml` scans recent run logs for credentials the masker did not redact. A scanner that cries wolf gets muted, so the fixtures in `scripts/test-actions-log-secret-scan.sh` also fail CI when a documented OpenAPI example carries a credential shape — a failing contract test dumps those examples into a public log, and the resulting alarm is indistinguishable from a leak. The same reasoning removed the other republishing route: `release.yml`'s `check-release` took `github.event.head_commit.message` through `env:`, so the runner reprinted every merged PR body into a public log and a credential shape quoted in one raised a High finding that no rotation could answer; the job now reads the subject from the checked-out commit, compares it as quoted data, and never receives the body through a log-visible environment entry. See [`ci-secret-handling.md`](ci-secret-handling.md) | MITIGATED |
...
| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-APIKEY-001 | API key brute force, timing oracle, or plaintext leak | Medium | Existing keys have 256-bit entropy and an `evr_app_` prefix; only the SHA-256 digest is stored. Inbound keys are hashed and compared in constant time after endpoint liveness is established. Endpoint and archival reads redact the digest. App key creation and regeneration surfaces are retired | MITIGATED |
| TM-APIKEY-002 | Cross-endpoint session access via a leaked session id | High | Every read, message, and cancel verifies the session carries the endpoint's permanent historical `app:` and `app_channel:` routing tags. A mismatch returns `404` so existence is not leaked. The session is bound to the resolved endpoint's Agent and Harness; the caller cannot select another Agent or pass management fields | MITIGATED |
| TM-APIKEY-003 | DoS via runaway execution-key client | Medium | Agent managers configure a per-endpoint, per-IP cap with `rate_limit_per_minute`. Enforcement runs after key comparison, uses the shared `ChannelRateLimiter` with a dedicated `apikey` namespace, and scopes buckets by the permanent historical App and endpoint IDs. The field is capped at `1_000_000`; `0` or absent falls back to the global API limit | MITIGATED |
...
| Slack action crosses its originating conversation | TM-SLACK-005 | Resolve trusted channel and thread metadata from session storage in the control plane; reject model-supplied alternate channels, channel-level uploads, and endpoint-scope mismatches before any Slack API request |
