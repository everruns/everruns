...
| TM-FS-017 | Local session resume catalog serializes Agent credentials or configuration | High | The Framework local catalog persists only typed session ids in owner-only SQLite state. Full session configuration remains in process and is rebuilt from the resuming Agent, so MCP headers/environment values, provider credentials, and initial-file content never enter the catalog. A credential-sentinel acceptance test scans every local profile file. | MITIGATED |
