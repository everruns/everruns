...
| TM-AUTHZ-018 | A user creates or replaces a service-owned MCP grant without authority over the MCP server or agent identity | High | Identity-mode authorize requires `MCP_SERVER_MANAGE` before discovery or identity creation. The callback re-evaluates the active permission resolver, resolves the MCP server and identity under the authenticated org, and accepts only active rows before storing an encrypted grant. The callback state carries the already-resolved identity id, so callback input cannot create a new identity. Permission denial, discovery/configuration failure, callback denial, archived-server, and state-mismatch tests fail before grant persistence. | MITIGATED |
...
| TM-OBS-012 | Service-owned MCP activity is recorded as the invoking user or accepts a forged acting principal | Medium | Before durable tool-started and tool-completed events are written, the event service resolves the effective scoped MCP attachment and the session's agent-identity principal under the session org. For `actsAs: service`, it overwrites `acting_principal_id` with that principal and preserves a supplied human `initiator_principal_id`; when the initiator is absent it records the session owner. Event metadata contains principal ids only, never OAuth credentials. | MITIGATED |
