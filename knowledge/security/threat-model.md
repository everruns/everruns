...
| TM-AUTHZ-005 | Anonymous channel reaches draft, disabled, or protected channel config | High | Public AG-UI ingress resolves `agent_channels JOIN agents`, then requires channel status `live`, channel enabled, Agent active, and Agent exposures not suspended before request work. Legacy configs then require `anonymous=true` plus the configured token when present. First-class channel auth bypasses the legacy anonymous flag only after the shared verifier accepts the configured credential policy. Nested `channel_config.auth` remains a read-only compatibility source and malformed stored policy fails closed. Failures return before session creation or image upload. | MITIGATED |
| TM-AUTHZ-006 | Anonymous webhook reaches a non-live channel | High | Public webhook ingress resolves liveness through `app_ingress::channel_liveness` — channel `status = live`, agent `active`, exposures not suspended — before the per-channel token is checked, and only then creates or reuses a session. Liveness precedes auth so a caller cannot distinguish a misconfigured channel from a bad token, and every not-live reason collapses to the same generic rejection. Replaced the `App.status × channel.enabled` gate in EVE-1007. | MITIGATED |
...
| TM-AUTHZ-019 | A retained App-shaped ingress alias bypasses Agent channel ownership or liveness | High | Every permanent `/v1/apps/{app_id}/...` alias resolves through `agent_channels.legacy_app_public_id` and `agents`; traffic-serving code does not read `apps` or the `app_channels` compatibility view. Alias and canonical channel routes share the channel-owned authentication, tenant, owner-principal, version, and liveness context. A DB-backed HTTP regression exercises every retained alias under a PostgreSQL role that is denied reads from both frozen App relations. | MITIGATED |
...
| TM-DOS-004 | Database connection pool exhaustion | Medium | Two sqlx pools per instance with asymmetric acquire timeouts (EVE-1081): a request pool (`DATABASE_POOL_MAX`, short `DATABASE_ACQUIRE_TIMEOUT_SECS`) that sheds load rather than queueing, and a small reserved background pool (`DATABASE_BACKGROUND_POOL_MAX`, long `DATABASE_BACKGROUND_ACQUIRE_TIMEOUT_SECS`) that background sweeps draw from via `StorageBackend::for_background()`. A single shared pool was **not** sufficient: one deadline for every consumer meant saturation from any source failed all of them at the same instant — prod EVERRUNS-13/15/16/B, with user-facing 500s on the same trace (EVERRUNS-1C). The split bounds the blast radius; it does not make the request path immune to a large enough burst, which is [EVE-1088](https://linear.app/everruns/issue/EVE-1088) | **PARTIAL** |
...
Agent channel-owned Agent2Agent (A2A) protocol ingress. JSON-RPC 2.0 endpoint authenticated by channel credentials. The canonical route is channel-scoped; the App-shaped route is a permanent alias. Mitigations live in `crates/server/src/api/app_a2a.rs`, `crates/server/src/domains/agent_channels/`, and the frozen invocation compatibility runtime. See `knowledge/integrations/a2a-channel.md`.

| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-A2A-001 | API key brute force | Medium | Existing keys have 256-bit entropy and an `evra2a_` prefix; only the SHA-256 hex digest is stored. Channel reads and the Agent Card never include plaintext or the digest. App key creation and regeneration surfaces are retired | MITIGATED |
...
| TM-A2A-004 | Anonymous ingress to draft, disabled, suspended, or inactive channels | High | Channel liveness checks run before key validation; the Agent Card endpoint mirrors the same gate and 404s otherwise | MITIGATED |
...
| TM-A2A-008 | Stored API key material leaks through channel management reads | Medium | Agent channel and archival App reads redact `api_key_hash`; only the non-secret display prefix is returned. The retired App key-generation and key-rotation operations no longer expose plaintext | MITIGATED |
...
| TM-A2A-013 | DoS via runaway A2A client | Medium | Agent managers can configure a per-channel, per-IP request cap with `rate_limit_per_minute`. Enforcement runs after credential verification, uses the shared `ChannelRateLimiter` with a distinct `a2a` namespace, and scopes buckets by the permanent historical App and channel IDs. The server caps the field at `1_000_000`; `0` or absent falls back to the global API limit | MITIGATED |
...
A2A reuses the frozen invocation compatibility runtime. Shared-session lookup requires the channel's resolved org, owner principal, and complete permanent routing tag set to match. A user-created session with spoofed tags is rejected.

## 23. FCP Channel (TM-FCP)

Agent channel-owned Free Communication Protocol ingress, with a permanent App-shaped route alias. Text-first HTTP endpoint with a deliberately minimal auth stack (anonymous + optional shared bearer token) and a dedicated `ChannelRateLimiter` namespace. Mitigations live in `crates/server/src/api/fcp.rs` and the Agent channel domain. See `knowledge/integrations/fcp-channel.md`.

| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-FCP-001 | Anonymous FCP request reaches a draft, disabled, suspended, or wrong-type channel | High | FCP resolution rejects an unknown channel, wrong transport, non-live or disabled channel, inactive Agent, or suspended Agent with the same generic 404 Markdown body. Caller cannot distinguish operator state, so the channel cannot be used as a probe oracle | MITIGATED |
...
| TM-FCP-003 | Configured bearer token leaked in response body or logs | High | The 401 and handshake bodies are static templates that never interpolate the configured token. Agent channel and archival App reads redact the token and surface `token_configured: true`; storage keeps it only in encrypted `channel_config`. Tracing does not log the token | MITIGATED |
| TM-FCP-004 | Cross-channel auth verifier reuse expands FCP attack surface | Medium | FCP deliberately does NOT call `ChannelAuthVerifier`. `normalize_and_validate_channel_config` rejects `channel_config.auth` for `ChannelType::Fcp`, so OIDC/HTTP-Basic/mTLS verifier code paths cannot run for FCP requests. New auth modes require an explicit code change, not a config flag | MITIGATED |
...
| TM-FCP-008 | Cross-org session reuse via FCP cookie spoofing | High | Channel resolution supplies the owning org and mandatory owner principal directly from `agent_channels JOIN agents`. Session reuse preserves the historical App attribution and routing tags, and still requires the resolved org, owner, and full tag set. A `fcp_session` cookie that does not match is ignored and a fresh channel-owned session is created | MITIGATED |
...
| TM-FCP-012 | Handshake (GET) used to probe operator existence | Low | The handshake is intentionally open per FCP SPEC, but resolution returns the same generic 404 body for unknown, draft, suspended, or wrong-type channels. The per-channel rate limiter also applies to GETs when configured | MITIGATED |

### Mitigation Details

**TM-FCP-003, Token redaction across read paths:**
The shared channel-config redaction path removes `token` and returns only `token_configured: true`. Agent channel updates merge preserved secret fields before encrypting the new configuration, so changing another field does not clear the token.

**TM-FCP-008, Session ownership invariant:**
The channel-owned execution context supplies the mandatory owner principal while retaining archival App attribution and the `fcp:app:<app_public_id>` tag for existing routes. The reuse path requires the resolved org, owner, and complete routing tag set to match, so guessing an alias or forging a cookie cannot adopt another channel's session.
...
Frozen execution-only API keys (`evr_app_...`) authenticate channel-owned native session routes. Canonical routes use `/v1/e/{channel_id}/sessions`; `/v1/apps/{legacy_app_id}/api/{channel_id}/...` remains a permanent alias. The key has no path to a management API. Mitigations live in `crates/server/src/api/app_api.rs`, `crates/server/src/domains/agent_channels/`, and the frozen invocation compatibility runtime. See `knowledge/integrations/app-api-keys.md`.
...
| TM-APIKEY-002 | Cross-channel session access via a leaked session id | High | Every read, message, and cancel verifies the session carries the channel's permanent historical `app:` and `app_channel:` routing tags. A mismatch returns `404` so existence is not leaked. The session is bound to the resolved channel's Agent and Harness; the caller cannot select another Agent or pass management fields | MITIGATED |
| TM-APIKEY-003 | DoS via runaway execution-key client | Medium | Agent managers configure a per-channel, per-IP cap with `rate_limit_per_minute`. Enforcement runs after key comparison, uses the shared `ChannelRateLimiter` with a dedicated `apikey` namespace, and scopes buckets by the permanent historical App and channel IDs. The field is capped at `1_000_000`; `0` or absent falls back to the global API limit | MITIGATED |
...
| TM-APIKEY-005 | Anonymous ingress to draft, disabled, suspended, or inactive channels | High | Channel liveness and channel-type checks run before key validation; non-live and unknown channels collapse to generic responses so the channel is not a probe oracle | MITIGATED |
| TM-APIKEY-006 | Cross-org session reuse via tag spoofing | High | Shared sessions are matched by channel-resolved org, owner principal, and complete permanent routing tag set. A user cannot pre-seed an `app:` or `app_channel:` tagged session for an execution-key invocation to reuse | MITIGATED |
