...
| ID | Threat | Severity | Mitigation | Status |
|----|--------|----------|------------|--------|
| TM-CI-001 | Workflow-level `DOPPLER_TOKEN` exfil via PR-controlled cargo build/test | Critical | `DOPPLER_TOKEN` is no longer declared at workflow `env:` in `ci.yml`, `brave-search-integration.yml`, `cursor-integration.yml`, or `sprites-integration.yml`. It is injected only into jobs/steps gated on `github.event_name == 'push'` (live-test jobs in ci.yml + per-integration live-test jobs). The Slack step in `integration-test` injects `DOPPLER_TOKEN` via step-scoped env only, guarded by `if: github.event_name == 'push'`; the `Run LLM integration tests` step injects it the same way inside `live-provider-matrix`, a job whose own condition is `push`-gated (EVE-938) | MITIGATED |
