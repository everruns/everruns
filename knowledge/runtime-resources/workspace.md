...
**Environment** at the host boundary. That model adds backend-owned mutable
...
mutable view. Physical paths, Git branches, and backend-specific revisions do
not enter the universal contract.

Workspace backends are an open async host SPI. Environment leaves compute and
...
head and does not claim writer isolation: compare-and-set failures and backend
status are the conflict surfaces. A session binding is persisted as bounded,
credential-free opaque backend data. Resume must reopen the recorded
backend/workspace/head identity or fail structurally; substitution is
forbidden.
...
Fork, checkpoint, status, archive, and destroy are explicit backend lifecycle
