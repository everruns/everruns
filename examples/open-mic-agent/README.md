...
![Open Mic Agent terminal demo](demo/demo.gif)
...
`Jev::from_env()` reads `TYPESAFE_API_KEY` and contributes one tool, `jev_evaluate`. It is the same tool the hosted [TypeSafe integration](https://docs.everruns.com/integrations/typesafe/) gives platform agents, so behavior matches whether you embed the Framework or run on Everruns.
...
The screencast runs the same `cargo run -q -p everruns-open-mic-agent` command shown above. VHS hides most provider wait time but does not replace the model, the classifier, or the tools with scripted output. Read `demo/transcript.txt` at your own pace.
