...
if [[ "${1:-}" == "--check" ]]; then
  grep -Fq 'cargo run -q -p everruns-framework-support-agent -- --interactive' "$tape"
  grep -Fq 'Wait+Screen /Question:/' "$tape"
  grep -Fq 'Type@35ms' "$tape"
  grep -Fq 'Wait+Line@120s />$/' "$tape"
  sample="$(printf '\033[0mMODEL: test\r\nANSWER\r\nCompleted: test\r\n> ' | extract_transcript /dev/stdin)"
  [[ "$sample" == $'MODEL: test\nANSWER\nCompleted: test' ]]
  if command -v vhs >/dev/null 2>&1; then
    vhs validate "$tape"
  fi
  echo "Everruns Support Agent demo checks passed"
  exit 0
fi
...
if [[ -n "${ANTHROPIC_API_KEY:-}" ]]; then
  ...
  command -v doppler >/dev/null 2>&1 || {
    echo "set ANTHROPIC_API_KEY or install Doppler to record the live demo" >&2
    exit 1
  }
