...
PAGE_LINES = 26
SECTION_LABELS = {"REQUEST", "SUMMARY", "AUDIT CHECKS ON DISK", "AUDIT.md AFTER THE RUN"}
...
def render():
    ...
    # Frames, not a GIF: VHS's own GIF output silently produces nothing in
    # some environments, while frame capture is reliable everywhere. record.sh
    # encodes the frames with ffmpeg, so one path is always the one exercised.
    tape = ["# Generated from demo.txt by render_demo.py.",
            "Output .demo-frames/", 'Set Shell "bash"', "Set Width 1440",
