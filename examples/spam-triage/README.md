...
The screencast runs the same `cargo run -q -p everruns-spam-triage` command shown above. VHS hides most provider wait time but does not replace the models with scripted output. Read `demo/transcript.txt` at your own pace.
