...
- **`content_type`** — Detected content type (`source`, `log`, `csv`, `config`, `text`, `minified`). See [Content-Type Defaults](#content-type-defaults).
- **`read_mode`** — `offset` (read from start/offset) or `tail` (read from end, for log files).
...
**Explicit parameters always win.** If the agent passes `offset` or `limit`, content-type defaults are bypassed entirely.
...
When sandbox tools (`bash`, `daytona_exec`, `e2b_exec`, etc.) produce output, the inline result is truncated based on the [`output` verbosity parameter](/specs/tool-execution#output-verbosity) (default: `concise` ~2 KiB). But the **full output is always persisted** to the session filesystem.
...
Every `read_file` response includes a `content_hash` (SHA-256). The `edit_file` tool requires this hash as a parameter — if the file has changed since the read, the edit is rejected.
...
// edit_file requires it — rejected if file changed
{
  "path": "/workspace/src/main.rs",
  "content_hash": "sha256:a1b2c3...",
  "old_text": "fn main() {",
