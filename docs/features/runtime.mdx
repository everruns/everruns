...
drive live sessions. If that runner cannot deliver to every session in the
organization, implement `routable_session_ids` with a current route snapshot;
an empty snapshot claims no schedules. Retain the handle for the host lifetime
...
The runner atomically claims due SQLite records only for routable sessions,
recovers stale claims after an interrupted process, advances timezone-aware
recurring schedules, and retries failed `send_message` delivery after the
configured claim timeout. Delivery is at-least-once across the narrow crash
window between the host accepting a message and SQLite recording its
completion.
