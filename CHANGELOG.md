...
- **Operator-safe defaults across server, daytona, CLI, and migrations** - Public `/metrics` is now off by default ([#1596](https://github.com/everruns/everruns/pull/1596)), Valkey auth rate-limit errors fail closed ([#1597](https://github.com/everruns/everruns/pull/1597)), admin seed promotion is blocked for untrusted email ([#1590](https://github.com/everruns/everruns/pull/1590)), the Daytona git-clone helper consults a configurable GitHub Enterprise trusted-host allowlist ([#1601](https://github.com/everruns/everruns/pull/1601)), the CLI exposes an opt-in `initial_files_allow_hidden` allowlist while keeping a hard-deny floor for credentials ([#1600](https://github.com/everruns/everruns/pull/1600)), and applied migrations are now enforced as immutable.
- **Client-side tools deprecation window** - Legacy non-`client_side` entries on `CreateSessionRequest.tools`, `CreateAgentRequest.tools`, and `UpdateAgentRequest.tools` are now dropped with a structured `tracing::warn!` instead of returning `400`, giving SDK/CLI consumers a soft-landing migration path ([#1602](https://github.com/everruns/everruns/pull/1602)). Set `EVERRUNS_REJECT_NON_CLIENT_SIDE_TOOLS=1` to opt back into hard-reject. Timeline in `specs/client-side-tools.md`.
- **Settings: dedicated organisations page** - The settings surface splits organisations into its own page for easier navigation and future per-org controls.
- **Capabilities: schema-driven config UI** - Capability config settings now render directly from their JSON schemas, reducing duplication between backend and UI.
