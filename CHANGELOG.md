...
- feat(model-router): add Model Router scaffolding ([EVE-397](https://linear.app/everruns/issue/EVE-397)). Introduces the durable design (`specs/model-router.md`), `ModelRouterId` (`mrtr_` prefix) in the typed-id schema, foundational migration `026_model_routers.sql` (tables `model_routers`, `model_router_routes`, `model_router_candidates`), and `ModelRouter` / `ModelRouterRoute` / `ModelRouterCandidate` / `ModelRouterStrategy` core types with structural validation (route-key format, strategy parse, candidate weight non-negative, `single`-strategy routes capped at one candidate, `rules`-strategy candidates must have a rules doc). Strategy enum covers `single`, `ordered_fallback`, `weighted`, `rules`, and `custom` per PRD. Storage trait, CRUD APIs, runtime resolver, harness/agent/session binding migrations, and UI ship as follow-up vertical slices on top of this foundation; existing concrete `default_model_id` behavior is preserved.
- feat(volumes): add `workspace_volumes` capability scaffolding ([EVE-396](https://linear.app/everruns/issue/EVE-396)). Introduces the durable design in `specs/volumes.md`, the `VolumeId` / `vol_` prefix in the typed-id schema, foundational migration `025_volumes.sql` (tables `volumes`, `volume_files`, `session_volume_mounts`), `Volume` / `VolumeFile` / `WorkspaceVolumesConfig` core types with structural mount-config validation (vol_-prefixed IDs, `/workspace` paths, no `..`, no `//`, no duplicate or overlapping paths), and the `workspace_volumes` capability registered in the builtins registry with `config_schema`, `config_ui_schema`, `dependencies = [session_file_system]`, `features = [file_system]`, `Medium` risk classification, and `validate_config` enforcement. Volume CRUD APIs, filesystem APIs, mount resolution at session creation, read/write integration with `session_file_system` tools, and UI ship as follow-up vertical slices on top of this foundation.
- docs(capabilities): document admin-only tier for `virtual_bash` and `web_fetch` (follow-up to [#1485](https://github.com/everruns/everruns/pull/1485) + [#1500](https://github.com/everruns/everruns/pull/1500)). The `virtual_bash` (Low → High) and `web_fetch` (Medium → High) elevations from PRs #1485 / #1500 are now an explicit product-tier decision rather than an emergent side-effect of risk-level constants: trust-boundary doc comments are recorded at the top of `crates/core/src/capabilities/virtual_bash.rs` and `crates/core/src/capabilities/web_fetch.rs`; the admin-only gate, the canonical create/update enforcement point (`check_high_risk_caps` in `crates/server/src/domains/agents/commands.rs`), the sibling import-path helper (`require_admin_for_high_risk` in `crates/server/src/api/agents.rs`), and grandfathering semantics for member-owned agents are documented under `specs/capabilities.md` ("Admin-Only Tier Decision") and cross-referenced from `specs/permissions.md` ("Capability Risk Gate"); `specs/threat-model.md` (TM-AGENT-005) now lists the full high-risk capability set. No behavior change — non-admin org members still cannot newly assign `virtual_bash` / `web_fetch` to an agent; existing member-owned agents that already had those capabilities continue to run.
