...
| `status` | enum | `pending`, `processed` (internal, not exposed in API) |
...
The `status` field tracks workflow processing state:
- `pending` - User message awaiting workflow processing (events not yet emitted)
- `processed` - Message has been processed by workflow

User messages are created with `pending` status. The workflow's `process-pending-messages` activity loads pending messages, emits SSE events, and marks them as `processed`. Non-user messages (assistant, tool_call, tool_result) are created directly with `processed` status since they're generated by the workflow itself.
...
1. POST /v1/agents/{id}/sessions/{id}/messages
   → Creates Message(role=user, status=pending, content: { text: "How much is 2+2?" })
   → Checks if workflow is running
   → If not running, triggers session workflow

2. Workflow starts
   → Updates Session(status=running)
   → Emits Event(session.started)

3. Process pending messages (process-pending-messages activity)
   → Loads pending user messages from database
   → Emits Event(message.user, data: { message_id: "...", content: [...] })
   → Marks messages as processed

4. LLM call starts (call-model activity)
   → Loads all messages from database
   → Emits Event(step.started)

5. LLM responds
   → Creates Message(role=assistant, status=processed, content: { text: "The answer is 4" })
   → Emits Event(step.finished)
   → Emits Event(message.created, data: { message_id: "..." })

6. Session cycle complete (ready for more messages)
   → Updates Session(status=pending)
   → Emits Event(session.finished)

User can send another message to continue the conversation.
If workflow is already running when user sends a new message,
the message is saved with status=pending and will be picked up
by the next process-pending-messages iteration.
```
