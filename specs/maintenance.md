...
- threat model and security testing in sync per `specs/security-testing.md`: every `MITIGATED` threat has coverage, durable failpoint tests pass (`cargo test -p everruns-durable --test failure_injection_test --features "failpoints,postgres-tests" -- --test-threads=1`), and `deny.toml` license/advisory checks pass
- DeepSec updated to the latest published version before scanning: `cd .deepsec && pnpm update deepsec@latest`, then `pnpm deepsec scan --project-id everruns`, `pnpm deepsec process --project-id everruns`, and review `pnpm deepsec report --project-id everruns` — file issues for any deferred true-positive findings
...
- crate documentation for every crate whose public surface changed: its `README.md` and crate-level rustdoc must still meet the crate documentation standard in `specs/code-organization.md` (abstract, ecosystem line, example, docs links, license; no publishing mechanics or internal `specs/` links; badges only on published crates)
