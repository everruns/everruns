...
2. **Built-in allowlist** — common dev-ecosystem assets are allowed by default. Examples: `.agents`, `.github`, `.vscode`, `.claude`, `.cursor`, `.mcp.json`, `.gitignore`, `.gitattributes`, `.editorconfig`, `.prettierrc*`, `.eslintrc*`, `.eslintignore`, `.nvmrc`, `.node-version`, `.python-version`, `.tool-versions`, `.dockerignore`, `.rubocop.yml`. The full list lives in `crates/cli/src/commands/agents.rs::ALLOWED_DOT_ENTRIES`.
3. **Per-agent opt-in** — the agent manifest may declare `initial_files_allow_hidden: [".mytool", ".otherproj"]` to extend the allowlist for project-specific tooling. Entries that match the hard-deny floor are silently filtered out of the opt-in. The `initial_files_allow_hidden` field is consumed locally and stripped from the upload payload.

A skipped hidden path emits a `Warning:` to stderr identifying the rejected path, the built-in allowlist, and the user opt-ins in effect. Symlinks pointing outside the base directory are skipped regardless of policy. See `specs/threat-model.md` (TM-FS-009) for the security rationale.
