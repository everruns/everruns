...
| TM-PLUGIN-004 | Resource exhaustion via oversized catalog or plugin archive (zip bomb) | Medium | Size cap on fetched `marketplace.json`; plugin tarballs have bounded compressed downloads, bounded whole-archive decompression and entry iteration before subdirectory filtering, and per-file (64 KB), retained-total (4 MB), and retained-file-count (256) caps; symlink/hardlink entries are dropped | MITIGATED |
