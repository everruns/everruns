...
| TM-AUTH-012 | OAuth account linking collision / pre-hijacking | High | `oauth_callback` only auto-links a provider identity to an existing same-email account after TM-AUTH-017 has accepted the provider identity and the existing Everruns account already has `email_verified=true`. Unverified local/password shadow accounts are refused (never silently merged), preventing a pre-registration attacker from retaining password access after the mailbox owner completes OAuth. The refusal returns `409` mapped to the `oauth_account_exists` login category ("sign in with your original method") rather than the transient "try again" copy — safe because the OAuth caller proved mailbox ownership, and it closes a recovery dead-end (TM-AUTH-028). Successful auto-linking also revokes the user's existing refresh tokens so sessions minted before the identity-link event do not survive the trust-boundary change. An email already bound to a *different* OAuth provider is refused (single-provider schema) rather than silently rebound. Residual risk: if an attacker controls the email at the provider and the existing Everruns email is already verified, they gain access — inherent to email as the identity key. | MITIGATED |
...
| TM-OBS-008 | Raw trajectory content leak via dataset export | High | Dataset export (`POST /v1/evals/{eval_id}/runs/{run_id}/dataset` enqueues; `GET …/dataset/{dataset_id}` fetches — `specs/dataset-export.md`) exports raw model-view message content, so both endpoints are gated by the dedicated `DATASET_EXPORT` policy (`dataset.export`, requires `OrgAgentsManage` + `OrgSessionsManage`) distinct from read-only `EVAL_VIEW`/`REPORT_VIEW`; org-scoped through `get_run` (cross-org runs are 404, see TM-TENANT-001/002) and `get_dataset` re-resolves the run and verifies the handle's `eval_run_id` before returning, so a guessed dataset id from another org/run is 404 (covered by `test_dataset_export_cross_org_returns_not_found`); always-on secret scrubbing strips credential patterns from every exported string before it is persisted; optional `redact_content` blanks message/tool content while preserving structure. Phase 2 adds an at-rest store: the scrubbed NDJSON is persisted on the org-scoped `eval_run_datasets` row (never raw un-scrubbed content), and the row's FK to `eval_runs` is `ON DELETE SET NULL` so deleting the run detaches its datasets | MITIGATED |
...
| TM-AGENT-005 | Capability escalation via agent creation | High | RiskLevel enum on Capability trait; high-risk capabilities (`a2a_agent_delegation`, `docker_container`, `daytona`, `e2b`, `deno`, `bashkit_shell`, `web_fetch`, `model_scout`) require Admin role to assign via API; gate is at create/update only, member-owned agents that already had high-risk capabilities are grandfathered (see `specs/capabilities.md` "Admin-Only Tier Decision") | MITIGATED |
...
| TM-DOS-030 | Detached-spawn governance side door (uncapped peer fan-out) | High | A detached spawn (`spawn_agent(lifetime=detached)`, `TASK_KIND_SESSION`, `parent_session_id = NULL`) is a lifecycle-independent peer, so it correctly resets nesting depth and was **not** counted by the subagent descendant caps (TM-DOS-027) — an uncapped loop of detached spawns could fan out unbounded. `spawn_create_and_wait` now runs a dedicated admission gate (`enforce_detached_spawn_caps`) before creating the peer session: it resolves the origin subagent-tree root (`root_session_for_subagent_tree` on the spawning session) and counts detached (`session`-kind) tasks anywhere under that root (a BFS that follows every task's `child_session_id`, so detached spawns made by subagents or by other detached peers are all attributed to the origin root). Exceeding `max_active_detached_tasks` (default 8 non-terminal peers) or `max_total_detached_tasks` (default 50 peer records) rejects the spawn with a `ToolError` naming the attempted count and cap. Caps are configurable per agent/org via the `subagents` capability config (`max_active_detached_tasks` / `max_total_detached_tasks`). Subagent descendant accounting (TM-DOS-027) is unchanged; the two budgets are independent. **Follow-up:** attributing detached LLM *spend* to the origin root budget (Policy 1 of EVE-767) and gating detached spawns behind session-creation authority (Policy 2) are tracked separately — this entry closes the unbounded-fan-out (count) hole; spend attribution for a detached peer still resolves to its own fresh budget root until the follow-up lands. | MITIGATED |
...
| TM-HOOK-002 | Hook-as-exfil-channel: a hook command makes outbound network calls to leak session state | High | The hook dispatcher builds its own `Bash` (`crates/core/src/hook_dispatch.rs`) without `.network(...)` or an HTTP transport, so `curl`/`wget` have no network path in the hook vocabulary even though the build now compiles bashkit's `http_client` feature. Outbound HTTP exists only on the `bashkit_shell` tool path, gated by per-capability `enable_http` config and routed through `EgressService` (see TM-BASH-003). Re-evaluate if hook dispatch ever opts into `configure_http` | MITIGATED |
