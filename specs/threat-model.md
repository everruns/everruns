...
| TM-A2A-005 | A2A method abuse beyond `message/send` and `message/stream` | Medium | Method allowlist of two (`message/send`, `message/stream`); all other methods return JSON-RPC `-32601 Method not found` without touching the session pipeline. The `message/stream` branch reuses the same auth + validation as `message/send` before opening the SSE stream | MITIGATED |
