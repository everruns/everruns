...
| TM-DOS-020 | Guardrail `mcp` check latency / payload amplification | Medium | The `mcp` check `server`/`tool` references are bounded to `MAX_MCP_REF_LEN` at compile time; content forwarded to the endpoint is capped at 2 000 bytes (UTF-8 char-boundary safe); at most `MAX_MCP_CALLS_PER_INVOCATION` (4) mcp calls per tool invocation, each with a hard `MCP_CHECK_TIMEOUT` (10 s) — maximum added latency per tool call 40 s; calls are serialized within a single hook and every failure mode fails open. A slow or unresponsive external guardrail cannot wedge a turn beyond the bounded timeout budget. | MITIGATED |
