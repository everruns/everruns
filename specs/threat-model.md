...
| TM-API-016 | Public-endpoint internal error leakage | High | AG-UI streaming `RUN_ERROR` payloads route every payload-phase error through `crates/server/src/api/public.rs::PublicError`, mapping internal codes to a stable public set (`rate_limited`, `service_unavailable`, `request_too_large`, `internal_error`); raw provider strings, model IDs, HTTP status codes, quota state, and stack traces never reach the wire. Universal fallback is `internal_error`. Pre-stream HTTP rejections (`bad_request`, `forbidden`, `not_found`, generic 500) keep their existing texts but already avoid internal detail. Other public endpoints (Slack webhook + manifest) inherit the same contract for any payload-phase errors they add. See `specs/public-endpoints.md` | MITIGATED |
...
| TM-AGENT-005 | Capability escalation via agent creation | High | RiskLevel enum on Capability trait; high-risk capabilities (`docker_container`, `daytona`, `e2b`, `deno`, `virtual_bash`, `web_fetch`) require Admin role to assign via API; gate is at create/update only, member-owned agents that already had high-risk capabilities are grandfathered (see `specs/capabilities.md` "Admin-Only Tier Decision") | MITIGATED |
