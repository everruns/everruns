...
| TM-A2A-012 | `tasks/get` / `tasks/cancel` cross-org reads or destructive actions | Medium | Both handlers reuse the same auth + channel + method gate as `message/send` and look up the underlying session with `get_session(auth.org_id, ...)`, so an API key from one org can never read or cancel a task on a session it does not own. State derivation only consults turn lifecycle events; raw prompts, tool args, and LLM outputs are never echoed back. Cancelling an already-terminal task is idempotent — the same task object is returned without a state transition | MITIGATED |
