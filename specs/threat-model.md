...
| TM-DOS-030 | Long-session message-history read amplification | Medium | The per-turn conversation-history read (`list_message_events*`, `crates/server/src/storage/repositories/events.rs`) can return thousands of rows for long sessions (observed 3 031 rows / 1.12 s, EVE-730), amplifying DB→app network, JSONB deserialization, and memory cost. Row count is capped by `MESSAGE_SAFETY_LIMIT` (5 000) and per-turn context limits; on top of that the read now uses the compact `MessageEventRow` projection (id, sequence, event_type, data), dropping the heavy `context`/`metadata`/`tags` JSONB from the wire and from deserialization for every row while returning identical messages. The predicate stays on the partial index `idx_events_messages` (migration 084) so DB execution is a bounded index range scan. A latency-threshold structured `WARN` (`SLOW_QUERY_WARN_MS`, default 500 ms) surfaces slow reads for host alerting without logging SQL params, event content, or PII. | MITIGATED |
