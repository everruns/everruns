...
| TM-AUTH-012 | OAuth account linking collision | Medium | `oauth_callback` links a provider identity to an existing account by email, but only after TM-AUTH-017 has rejected `email_verified=false`, so the provider has proven the caller owns the email (same trust basis as email password reset). Linking preserves `password_hash`, so it cannot lock a user out of password login. An email already bound to a *different* OAuth provider is refused (single-provider schema) rather than silently rebound. Residual risk: if an attacker controls the email at the provider they gain access — inherent to email as the identity key. | **CALLER RISK** |
