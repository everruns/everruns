...
| TM-WEB-010 | Credential/DOM theft or forced navigation via HTML file preview (user-supplied `.html`/`.htm` that runs JS) | High | `HtmlPreview` (`apps/ui/src/components/files/file-previews.tsx`) renders HTML inside `<iframe sandbox="allow-scripts" srcDoc=... referrerPolicy="no-referrer">`. `allow-scripts` runs page JS but the omitted `allow-same-origin` forces an opaque origin: `document.cookie`, `localStorage`, and `parent.document` all throw `SecurityError` (verified). Omitting `allow-top-navigation`/`allow-forms`/`allow-popups`/`allow-modals` blocks redirects, form posts, popups, and dialogs. A defense-in-depth CSP meta (`object-src 'none'; base-uri 'none'; form-action 'none'`) is injected at the start of `<head>`; the opaque origin — not the CSP — is the primary gate. The opaque sandboxed `srcdoc` does not inherit the parent's `script-src 'self'` (same mechanism MCP cards rely on, `specs/mcp-cards.md`), so previews still run JS. Regression tests in `apps/ui/src/__tests__/file-previews.test.tsx`. | MITIGATED |
