...
|----|--------|----------|------------|--------|
| TM-AUTH-001 | Brute force login | High | Per-IP rate limiting on auth endpoints (login 10/min, register 5/min, refresh 30/min; OAuth redirect/callback share the login tier) plus a per-account throttle keyed on the submitted email across all IPs (20 attempts / 15 min — caps failed distributed credential stuffing without rejecting correct credentials, preventing targeted account lockout); dual backend: in-memory governor or Valkey distributed sliding-window | MITIGATED |
