...
- **Trigger.** Any capability whose `risk_level()` returns `High`. Today: `docker_container`, `daytona`, `e2b`, `deno`, `bashkit_shell`, `web_fetch`. The full contract — including the rationale for `bashkit_shell` / `web_fetch` and migration semantics for grandfathered agents — lives in `specs/capabilities.md` ("Admin-Only Tier Decision").
