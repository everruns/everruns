...
- Works with any backend (Jaeger, Zipkin, Datadog, Braintrust, etc.)
- Configure via `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable
- Full control over trace data and retention
- Users provide their own collector/backend (not bundled)

**May revisit when**:
- OpenAI officially documents the traces API for external use
- OpenAI explicitly supports custom/third-party trace ingestion

## Bundled Jaeger in Docker Compose

**Status**: Dismissed (removed after evaluation)

**What it was**: Jaeger was included in the local development and example Docker Compose files as a default trace collector, with OTEL_EXPORTER_OTLP_ENDPOINT pre-configured to send traces to it.

**Why considered**: Provided out-of-the-box distributed tracing visualization for local development without additional setup. Developers could immediately see traces in the Jaeger UI at localhost:16686.

**Why dismissed**:
- Added unnecessary infrastructure for most development workflows
- Most developers don't need tracing during local development
- Increased Docker resource usage and startup time
- Observability preferences vary (some prefer Datadog, Honeycomb, Braintrust, etc.)
- OpenTelemetry already supports pluggable backends

**What we use instead**:
- OpenTelemetry support remains (configure via `OTEL_EXPORTER_OTLP_ENDPOINT`)
- Users who need tracing can add their preferred collector
- Braintrust integration available for LLM-specific observability
- Simpler default setup with just PostgreSQL

**May revisit when**:
- There's strong user demand for built-in tracing
- A lighter-weight tracing solution emerges
