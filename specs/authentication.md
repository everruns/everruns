...
Token model: the raw token is emailed once and never stored; only its SHA-256 hash is persisted (`password_reset_tokens`, migration 088). Single-use is enforced via `used_at` set in one atomic `UPDATE … WHERE used_at IS NULL AND expires_at > now()`.
...
Token model is identical to password reset (hashed, single-use, short TTL; `email_verification_tokens`, migration 088) but with a 24-hour TTL. Both recovery and verification routes share the registration rate limiter (per client IP).
