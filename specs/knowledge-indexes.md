...
* **Knowledge Bases** (`specs/knowledge-bases.md`) — human-curated short
...
* **Memory** (`specs/memory.md`) — org-scoped file stores mounted into
...
* **Embeddings** — `ServiceKind::Embeddings` + `EmbeddingsDriver`, resolved via
  `resolve_service(org, ServiceKind::Embeddings, binding)` (`specs/providers.md`).
...
`specs/models.md`: `active` → `archived` (read-only, hidden from default lists,
...
Full DDL: `crates/server/migrations/071_knowledge_indexes.sql`. **Embedding
...
REST endpoints (conventions in `specs/apis.md`), shipped with the management
slice:
...
Canonical entries live in `specs/threat-model.md`.
