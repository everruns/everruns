...
> [runtime](runtime.md). It covers the shared client crate, transport
...
2. **Non-HTTP transport, hard-off in hosted capabilities.** stdio is supported
   ...
   hosted threat model).
3. **Reuse, not reinvention.** MCP remains a [virtual capability](capabilities.md);
...
belt-and-suspenders secondary check. See [threat-model.md](threat-model.md)
