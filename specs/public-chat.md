...
| "Isolated, no internal leaks" | Public-endpoint error sanitization ([public-endpoints.md](public-endpoints.md)) |
| "Sessions per visitor" | App-channel session ownership + routing tags ([app-invocation-channels.md](app-invocation-channels.md)) |
| "Org/app isolation" | Organization multitenancy ([multitenancy.md](multitenancy.md)) |
| "Reusable, brandable chat UI" | `apps/ui` chat components + `design-system.css` (already authored as the white-label source of truth) |
| "Own domain later" | Production reverse-proxy contract ([production-deployment.md](production-deployment.md)) |
...
5. **No internal-state leaks.** All public responses — success and error —
   ...
   ([public-endpoints.md](public-endpoints.md)). Tool surfacing follows the
...
4. **Cost guardrails (exists + thin add).** Public traffic runs under the App's
   ...
   ([budgeting.md](budgeting.md), [usage-tracking.md](usage-tracking.md)). MVP
...
contract ([production-deployment.md](production-deployment.md)) plus
...
- [public-endpoints.md](public-endpoints.md) — public error-sanitization contract
- [fcp-channel.md](fcp-channel.md) — isolation invariants for a public channel
- [multitenancy.md](multitenancy.md) — organization isolation
- [budgeting.md](budgeting.md) — spend bounds for public traffic
- [usage-tracking.md](usage-tracking.md) — LLM token usage tracking
- [authentication.md](authentication.md) — platform auth modes (contrast: visitor auth is separate)
- [production-deployment.md](production-deployment.md) — reverse-proxy contract (custom domains, future)
- [brand.md](brand.md) — brand identity, colors, typography
